...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasColumn('location_services', 'service_sub_service_id')) {
            Schema::table('location_services', function (Blueprint $table) {
                $table->unsignedBigInteger('service_sub_service_id')->default(0)->after('service_id');
            });
        }

        DB::table('location_services')->whereNull('service_sub_service_id')->update(['service_sub_service_id' => 0]);

        $indexes = $this->locationServiceIndexNames();

        if (! in_array('loc_svc_sub_provider_unique', $indexes, true)) {
            ...
            Schema::table('location_services', function (Blueprint $table) {
                $table->unique(
                    ['location_id', 'service_id', 'service_sub_service_id', 'provider_type'],
                    'loc_svc_sub_provider_unique'
                );
            });
        }
    }
    ...
    public function down(): void
    {
        ...
        if (in_array('loc_svc_sub_provider_unique', $indexes, true)) {
            Schema::table('location_services', function (Blueprint $table) {
                $table->dropUnique('loc_svc_sub_provider_unique');
            });
        }
        ...
        if (Schema::hasColumn('location_services', 'service_sub_service_id')) {
            Schema::table('location_services', function (Blueprint $table) {
                $table->dropColumn('service_sub_service_id');
            });
        }
    }
