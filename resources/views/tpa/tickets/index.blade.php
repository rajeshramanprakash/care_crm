                    ...
                    <div class="card">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'all' ? 'active' : '' }}" href="{{ route('tpa.tickets.index', ['status' => 'all']) }}">
                                        All Tickets
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'active' ? 'active' : '' }}" href="{{ route('tpa.tickets.index', ['status' => 'active']) }}">
                                        Active
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'closed' ? 'active' : '' }}" href="{{ route('tpa.tickets.index', ['status' => 'closed']) }}">
                                        Closed
                                    </a>
                                </li>
                            </ul>
...
$(document).ready(function() {
    ...
    function loadStatistics() {
        $.get('{{ route("tpa.tickets.unread-count") }}', function(data) {
            $('#unread-messages').text(data.unread_count || 0);
        });

        // Load ticket counts
        $.get('{{ route("tpa.tickets.ajax") }}', {status: '{{ $status }}'}, function(data) {
