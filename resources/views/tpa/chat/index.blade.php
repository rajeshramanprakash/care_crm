...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/tpa/chat/favorites').then(res => res.json()),
                        fetch('/tpa/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/tpa/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/tpa/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/tpa/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('tpa/chat/messages') }}/${userId}`;
            ...
            fetch(apiUrl)
                ...
                `;
                    ...
                    `;
                        ...
                        let repliedHtml='';
                        if(message.replied_to){const rT=message.replied_to.message||'[Attachment]';const rS=message.replied_to.sender?message.replied_to.sender.f_name:'Unknown';repliedHtml=`<div class="replied-message" onclick="scrollToOriginalMessage(${message.reply_to_id})"><div class="replied-message-sender">${rS}</div><div class="replied-message-text">${rT}</div></div>`;}
                        let messageHtml = '';
        ...
        function markAsRead(userId) {
            fetch(`{{ url('tpa/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('tpa.chat.send.attachment') }}';
                ...
                .then(messages => {
                    ...
                    if (messages.length === 0) {
                        ...
                        messages.forEach((msg, index) => {
                            ...
                            let repliedHtml='';
                            if(msg.replied_to){const rT=msg.replied_to.message||'[Attachment]';const rS=msg.replied_to.sender?msg.replied_to.sender.f_name:'Unknown';repliedHtml=`<div class="replied-message" onclick="scrollToOriginalMessage(${msg.reply_to_id})"><div class="replied-message-sender">${rS}</div><div class="replied-message-text">${rT}</div></div>`;}
                            let messageContent = '';
                            if (msg.attachment) {
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any tpas
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('tpa.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("tpa.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
