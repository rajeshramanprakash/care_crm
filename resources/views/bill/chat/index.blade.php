    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                cancelReply();
                userHasScrolledManually = false; // Reset scroll flag
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/billing/chat/favorites').then(res => res.json()),
                        fetch('/billing/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/billing/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/billing/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/billing/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('billing/chat/messages') }}/${userId}`;
            ...
            fetch(apiUrl)
                ...
                `;
                    ...
                    `;
                        ...
                        // Build replied message HTML if this is a reply
                        let repliedHtml = '';
                        if (message.replied_to) {
                            ...
                            repliedHtml = `
                                <div class="replied-message" data-original-message-id="${message.reply_to_id}" onclick="scrollToOriginalMessage(${message.reply_to_id})">
                                    <div class="replied-message-sender">${repliedSender}</div>
                                    <div class="replied-message-text">${repliedText}</div>
                                </div>
                            `;
                        }
                        ...
                        if (message.attachment) {
                            let attachmentHtml = '';
                            // Get display name with spaces for download attribute
                            ...
                            if (message.attachment_type && message.attachment_type.startsWith('image/')) {
                                attachmentHtml = `
                                    <div style="position:relative; display:inline-block;">
                                        ...
                                        <a href="/storage/${message.attachment}" download="${displayName}" class="btn btn-sm btn-primary" style="position:absolute;bottom:8px;right:8px;z-index:2; border-radius:50%; background:#F7941D; color:#fff; padding:8px;">
                                ...
                                attachmentHtml = `
                                    <div style="background:#e6f0ff; border-radius:8px; padding:16px; display:flex; align-items:center; gap:12px; color:#000;">
                                        ...
                                        <span style="flex:1;">${escapedDisplayName}</span>
                            ...
                            const messageTextForReply = '[Attachment]';
                            messageHtml = `
                        <div class="message ${isSent ? 'sent' : 'received'}" data-message-id="${message.id}">
                            ...
                            ${seenHtml}
                            <button class="message-reply-btn" data-message-id="${message.id}" data-sender-name="${senderName}" data-message-text="${messageTextForReply}">
                    ...
                    `;
                        ...
                        <div class=\"message ${isSent ? 'sent' : 'received'}\" data-message-id=\"${message.id}\">
                            ...
                            <div class=\"message-time\">${messageDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                            ${seenHtml}
                            <button class=\"message-reply-btn\" data-message-id=\"${message.id}\" data-sender-name=\"${senderName}\" data-message-text=\"${message.message}\">
                                <i class=\"fa fa-reply\"></i> Reply
                            </button>
                        </div>
                    `;
        ...
        function markAsRead(userId) {
            fetch(`{{ url('billing/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
            fetch('{{ route('bill.chat.unread-counts') }}')
                .then(response => response.json())
                .then(counts => {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('bill.chat.send.attachment') }}';
                ...
                .then(messages => {
                    ...
                    if (messages.length === 0) {
                        ...
                        messages.forEach((msg, index) => {
                            ...
                            let repliedHtml = '';
                            if (msg.replied_to) {
                                const repliedText = msg.replied_to.message || '[Attachment]';
                                const repliedSender = msg.replied_to.sender ? msg.replied_to.sender.f_name : 'Unknown';
                                repliedHtml = `
                                    <div class="replied-message" data-original-message-id="${msg.reply_to_id}" onclick="scrollToOriginalMessage(${msg.reply_to_id})">
                                        <div class="replied-message-sender">${repliedSender}</div>
                                        <div class="replied-message-text">${repliedText}</div>
                                    </div>
                                `;
        ...
        // Scroll to original message when replied message is clicked
        function scrollToOriginalMessage(originalMessageId) {
            const targetMessage = document.querySelector(`.message[data-message-id="${originalMessageId}"]`);
            if (targetMessage) {
                const chatMessages = document.getElementById('chatMessages');
                ...
                const scrollTop = chatMessages.scrollTop;
                const targetScrollTop = scrollTop + (targetRect.top - chatRect.top) - (chatRect.height / 2) + (targetRect.height / 2);
                ...
                // Reset flag after 10 seconds to allow auto-scroll again
                setTimeout(() => {
        ...
        document.addEventListener('click', function(event) {
            if (event.target.classList.contains('message-reply-btn') || 
                event.target.parentElement.classList.contains('message-reply-btn')) {
                const button = event.target.classList.contains('message-reply-btn') ? 
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any bills
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
            ...
            $(document).on('click', '.favorite-star', function(e) {
                e.stopPropagation();
                var userId = $(this).data('userId');
                fetch('/bill/chat/favorite/' + userId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
        ...
        function loadMessages(userId) {
            ...
            console.log('Starting API call for userId:', userId);
            const apiUrl = `{{ url('bill/chat/messages') }}/${userId}`;
            console.log('API URL:', apiUrl);
        ...
        function markAsRead(userId) {
            fetch(`{{ url('bill/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('bill.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("bill.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
