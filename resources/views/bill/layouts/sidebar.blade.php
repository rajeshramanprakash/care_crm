        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('bill.courier.index')}}" class="nav-link {{$uri == "courier" ? 'active' : ''}}">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('bill.query.index')}}" class="nav-link {{$uri == "query" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-shipping-fast"></i>
                        <p>Query</p>
                    </a>
                    @endif
                ...
                <li class="nav-item">
                    <a href="{{route('bill.chat.index')}}" class="nav-link {{$uri == "chat" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-comments"></i>
                        <p>
                            Chat
                            <span id="sidebar-unread-badge-bill" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#25d366; color:#fff; font-weight:bold;"></span>
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    @if($auth_user->is_ticket_enabled)
                    <a href="{{route('bill.tickets.index')}}" class="nav-link {{ strpos($route_name, 'tickets') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-ticket-alt"></i>
                        <p>
                            Tickets
                            <span id="sidebar-ticket-badge-bill" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#ffc107; color:#000; font-weight:bold;"></span>
                        </p>
                    </a>
                    @endif
                </li>
...
function updateSidebarUnreadBadgeBill() {
        fetch('{{ route('bill.chat.unread-counts') }}')
            .then(response => response.json())
            .then(counts => {
                let total = 0;
                Object.values(counts).forEach(count => total += count);
                const badge = document.getElementById('sidebar-unread-badge-bill');
                if (badge) {
                    if (total > 0) {
    ...
    updateSidebarUnreadBadgeBill();
        ...
        if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
            ...
            console.log('Sidebar real-time subscribed for updateSidebarUnreadBadgeBill');
            ...
            setInterval(updateSidebarUnreadBadgeBill, 30000);
        }
    }
    initSidebarRealtime();

    @if($auth_user->is_ticket_enabled)
    function updateSidebarTicketBadgeBill() {
        fetch('{{ route('bill.tickets.statistics') }}')
            .then(response => response.json())
            .then(stats => {
                const badge = document.getElementById('sidebar-ticket-badge-bill');
                if (badge) {
    ...
    updateSidebarTicketBadgeBill();
    setInterval(updateSidebarTicketBadgeBill, 10000);
    @endif
