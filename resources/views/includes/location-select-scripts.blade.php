...
(function ($) {
    ...
    function formatLocationOption(state) {
        ...
        return $('<span class="loc-opt-row"><span class="loc-opt-city">' + escapeHtml(city) + '</span><span class="loc-opt-state">' + escapeHtml(st) + '</span></span>');
    }
    ...
    window.initLocationSelects = function (context) {
        ...
        $scope.find('select.location-select').each(function () {
            ...
            if ($el.prop('required') || $el.data('allow-clear') === false) { opts.allowClear = false; }
            var $modal = $el.closest('.modal');
            if ($modal.length) { opts.dropdownParent = $modal; }
            $el.select2(opts);
            $el.next('.select2-container').addClass('location-select2');
            // Ensure typing goes into the dropdown search as soon as it opens.
    ...
    // Global fallback for any Select2 open (covers re-inits / doctor city too).
    ...
    $(document).ready(function () { initLocationSelects(); });
})(jQuery);
</script>
@endonce
