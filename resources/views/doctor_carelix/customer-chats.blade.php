    ...
    <div class="container-fluid d-flex justify-content-center align-items-start">
        <div class="chat-card row w-100 justify-content-center align-items-stretch mobile-chat-wrapper" style="overflow: hidden;">
            ...
            <div class="col-md-4 col-lg-4 chat-sidebar p-0 d-flex flex-column border-end mobile-user-list" style="background: #f9f9f9; min-width: 270px; max-width: 340px;">
                ...
                <div class="chat-user-list overflow-auto flex-grow-1" id="customersList" style="background: #f9f9f9;">
                    @forelse($customers as $customer)
                        <div class="chat-user-item d-flex align-items-center px-3 py-2" data-customer-id="{{ $customer->id }}" data-linked-leads="{{ $customer->doctor_customer_linked_lead_ids_json ?? '[]' }}" data-profile-image="{{ $customer->profile_image ? Storage::url($customer->profile_image) : '' }}" data-initial="{{ strtoupper(substr($customer->customer_name ?? 'C', 0, 1)) }}" style="text-decoration:none; cursor: pointer;">
                    ...
                    @empty
                        <div class="text-center p-4 text-muted">
                            ...
                            <p>No customers yet — chat opens after a website consultation booking.</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="col-md-8 col-lg-8 chat-main p-0 d-flex flex-column mobile-chat-area" style="background: #fff;">
                ...
                <div id="doctorChatScheduleBanner" class="alert alert-warning mb-0 rounded-0 d-none py-2 px-3" style="border-radius: 0 !important; border-left: none; border-right: none; margin: 0;" role="alert"></div>
                <div class="chat-messages flex-grow-1 position-relative" id="chatMessagesContainer" style="overflow-y: auto; padding: 32px; min-height: 350px; max-height: 60vh;">
...
<style>
    ...
    .chat-card { background: transparent; overflow: hidden; height: 83vh; min-height: 0; }
    .chat-sidebar { background: #f9f9f9; border-right: 1px solid #f0f0f0; border-radius: 18px 0 0 18px; min-height: 0; height: 100%; }
...
@endphp
<script>
window.__doctorCustomerRealtimeConfig = @json($__dcReverbCfg);
window.__doctorCustomerRealtimeSignatures = @json($doctorCustomerRealtimeSignatures ?? []);
window.__doctorPortalDoctorRequestId = @json((int) \Illuminate\Support\Facades\Session::get('doctor_reg_id'));
/** HTTP poll interval (ms): keep ~2.5s — never slow to 15s when Echo \"connects\"; broadcast often fails silently. */
window.__doctorCustomerRealtimePollMs = 2500;
</script>
...
/** False when consultation appointment window has not started / has ended — server sends chat_schedule on each messages fetch. */
let doctorChatScheduleAllowed = true;

function applyDoctorChatScheduleBanner(schedule) {
    const $b = $('#doctorChatScheduleBanner');
    if (!$b.length) return;
    if (schedule && schedule.allowed === false) {
        $b.removeClass('d-none').text(
            schedule.user_message || 'You can chat only during your scheduled appointment time.'
        );
    } else {
...
function updateDoctorChatComposerLock() {
    const hasCustomer = !!currentCustomerId;
    const locked = hasCustomer && !doctorChatScheduleAllowed;
    $('#messageInput').prop('disabled', !hasCustomer || locked);
    $('#attachmentBtn').prop('disabled', !hasCustomer || locked);
    $('#messageForm button[type="submit"]').prop('disabled', !hasCustomer || locked);
}
...
$(document).ready(function() {
    ...
    // Customer selection (use .attr so IDs are always strings from DOM; jQuery .data() can coerce/cache oddly)
    $(document).on('click', '.chat-user-item', function(e) {
        if ($(e.target).closest('a[href]').length) return;
        const customerIdRaw = $(this).attr('data-customer-id');
        const customerId = customerIdRaw ? String(customerIdRaw).trim() : '';
        ...
        doctorChatScheduleAllowed = true;
        applyDoctorChatScheduleBanner(null);
        updateDoctorChatComposerLock();
        $('.chat-welcome').hide();

        // Load messages
        loadMessages(customerId, true); // Initial load, scroll to bottom

        // Start polling for messages (slow when Reverb/Echo delivers live updates)
    ...
    $('#messageForm').on('submit', function(e) {
        ...
        if (!doctorChatScheduleAllowed) {
            alert('You can chat only during the scheduled appointment date and time from your booking.');
            return;
        }
        ...
        $.ajax({
            url: '{{ route("doctor_portal.customer-chats.send") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                ...
                if (String(currentCustomerId) === String(receiverId)) {
                    loadMessages(receiverId, true, true); // Force scroll and force reload when sending
                }
            },
            error: function(xhr) {
                console.error('Error sending message:', xhr);
                const msg = (xhr.responseJSON && (xhr.responseJSON.error || xhr.responseJSON.message)) || 'Failed to send message';
                alert(msg);
            }
        });
    });
    ...
    function updateUnreadCounts() {
        $.get('{{ route("doctor_portal.customer-chats.unread-counts") }}')
    ...
    function loadMessages(customerId, shouldScroll = false, forceReload = false) {
        ...
        let url = '{{ route("doctor_portal.customer-chats.messages", ":id") }}'.replace(':id', customerId);
        ...
        $.get(url)
            .done(function(payload) {
                if (String(currentCustomerId) !== String(customerId)) return;
                ...
                if (Array.isArray(payload)) {
                    ...
                    items = payload.items;
                    const schedule = payload.chat_schedule || null;
                    doctorChatScheduleAllowed = !schedule || schedule.allowed !== false;
                    applyDoctorChatScheduleBanner(schedule);
                } else {
                    console.error('loadMessages: unexpected response', payload);
                    $('#chatMessages').html(
                        '<div class="alert alert-warning m-3">Could not load messages. If you are logged out, refresh the page and sign in again.</div>'
                    );
                    return;
                }
                ...
                items.forEach(function(item) {
                    if (item.type === 'call') {
                        ...
                        const isOutgoing = call.caller_type === 'doctor';
                    ...
                    const isSent = msg.sender_type === 'doctor';
            ...
            .fail(function(xhr) {
                ...
                if (String(currentCustomerId) === String(customerId)) {
                    $('#chatMessages').html(
                        '<div class="alert alert-danger m-3">Failed to load messages (' + (xhr.status || '?') + '). Try again or refresh the page.</div>'
                    );
                }
            });
    }
    ...
    function markAsRead(customerId) {
        $.post('{{ route("doctor_portal.customer-chats.mark-read", ":id") }}'.replace(':id', customerId), {
            _token: '{{ csrf_token() }}'
        });
    }
    ...
    $(document).on('click', '.message-translate-btn', function(e) {
        ...
        $.ajax({
            url: '{{ route("doctor_portal.customer-chats.translate") }}',
            type: 'POST',
    ...
    if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
        const realtimeUserId = @json(Auth::id());
        if (realtimeUserId != null) {
            window.ChatRealtime.subscribeUserChannel(realtimeUserId, function(payload, eventName) {
                updateUnreadCounts();
            });
        }
    ...
    function checkForIncomingCall() {
        ...
        $.get('{{ route("doctor_portal.customer-chats.call.offer") }}', { customer_id: currentCustomerId })
    ...
    function sendCallOffer(offer) {
        $.ajax({
            url: '{{ route("doctor_portal.customer-chats.call.offer.send") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    function pollForCallAnswer() {
        const answerInterval = setInterval(() => {
            if (!isCallActive && peerConnection) {
                $.get('{{ route("doctor_portal.customer-chats.call.answer.get") }}', {
                    customer_id: currentCustomerId
                })
                .done(function(response) {
                    if (response.answer) {
    ...
    function sendCallAnswer(answer, customerId) {
        $.post('{{ route("doctor_portal.customer-chats.call.answer") }}', {
            _token: '{{ csrf_token() }}',
    ...
    function sendIceCandidate(candidate, customerId) {
        $.post('{{ route("doctor_portal.customer-chats.call.ice.send") }}', {
            _token: '{{ csrf_token() }}',
            customer_id: customerId,
    ...
    function endCall() {
        ...
        if (customerIdToEnd) {
            $.post('{{ route("doctor_portal.customer-chats.call.end") }}', {
                _token: '{{ csrf_token() }}',
    ...
    setInterval(function() {
        if (isCallActive && peerConnection && currentCustomerId) {
            ...
            $.get('{{ route("doctor_portal.customer-chats.call.ice") }}', { customer_id: customerId })
                .done(function(response) {
    ...
    function dcWsCustomerOlId(payload) {
        if (!payload || !payload.message) return null;
        const row = payload.message;
        if (row.sender_type === 'customer') return String(row.sender_id);
        if (row.receiver_type === 'customer') return String(row.receiver_id);
        return null;
    }

    function dcWsTargetsOpenDoctorChat(payload) {
        if (!payload || !currentCustomerId) return false;
        const myDoc = Number(window.__doctorPortalDoctorRequestId || 0);
        if (myDoc > 0 && Number(payload.doctor_request_id || 0) !== myDoc) return false;
        const ol = dcWsCustomerOlId(payload);
        if (!ol) return false;
        const active = $('.chat-user-item.active');
        let linked = [];
        try {
            linked = (JSON.parse(active.attr('data-linked-leads') || '[]') || []).map(String);
        } catch (e) {
            linked = [];
        }
        return linked.indexOf(ol) !== -1 || String(currentCustomerId) === ol;
    }

    (function initDoctorCustomerReverb() {
        var cfg = window.__doctorCustomerRealtimeConfig || {};
        var sigs = window.__doctorCustomerRealtimeSignatures || [];
        ...
        try {
            window.__dcDoctorCustomerEcho = window.__dcDoctorCustomerEcho || new EchoCtor({
                broadcaster: 'reverb',
                key: cfg.key,
                cluster: '',
                wsHost: cfg.wsHost,
            ...
            sigs.forEach(function (sig) {
                if (!sig) return;
                var chName = prefix + sig;
                window.__dcDoctorCustomerEcho.channel(chName)
                    .listen('.doctor.customer.message', function (e) {
                        updateUnreadCounts();
                        if (dcWsTargetsOpenDoctorChat(e) && currentCustomerId) {
                            loadMessages(currentCustomerId, true, true);
                        }
                    })
                    .listen('.doctor.customer.unread', function () {
                        updateUnreadCounts();
                        if (currentCustomerId) {
                            loadMessages(currentCustomerId, false, true);
                        }
                    });
            });
            // Do not reduce poll when Echo loads — Reverb/broadcast may still be broken; fast poll is the reliable path.
        } catch (err) {
            if (window.console && console.warn) {
                console.warn('Doctor-customer Reverb init failed:', err);
            }
        }
    })();
});
