...
@section('title', isset($page_heading) ? $page_heading : 'Customer Chats')
            ...
            <div class="col-md-4 col-lg-4 chat-sidebar p-0 d-flex flex-column border-end mobile-user-list" style="background: #f9f9f9; min-width: 270px; max-width: 340px;">
                ...
                <div class="chat-user-list overflow-auto flex-grow-1" id="customersList" style="background: #f9f9f9;">
                    @forelse($customers as $customer)
                        <div class="chat-user-item d-flex align-items-center px-3 py-2" data-customer-id="{{ $customer->id }}" data-profile-image="{{ $customer->profile_image ? Storage::url($customer->profile_image) : '' }}" data-initial="{{ strtoupper(substr($customer->customer_name ?? 'C', 0, 1)) }}" style="text-decoration:none; cursor: pointer;">
            ...
            <div class="col-md-8 col-lg-8 chat-main p-0 d-flex flex-column mobile-chat-area" style="background: #fff;">
                <div class="chat-header d-flex align-items-center px-2 py-2 border-bottom bg-white">
                    ...
                    <div class="position-relative me-3">
                        <img id="chatHeaderAvatar" src="" alt="Customer" class="rounded-circle chat-user-avatar" style="display: none; width: 44px; height: 44px; object-fit: cover; border: 2px solid #F7941D;" onerror="this.style.display='none'; var fb=document.getElementById('chatHeaderAvatarFallback'); if(fb) fb.style.display='flex';">
                ...
                <div class="chat-messages flex-grow-1 position-relative" id="chatMessagesContainer" style="background: #f8f9fa; overflow-y: auto; padding: 32px; min-height: 350px; max-height: 60vh;">
...
<style>
    ...
    .chat-card { background: transparent; overflow: hidden; height: 83vh; }
    ...
    .chat-messages { background: #f8f9fa; overflow-y: auto; flex-grow: 1; padding: 32px; min-height: 350px; max-height: 60vh; display: flex; flex-direction: column; }
...
$(document).ready(function() {
    ...
    $('#messageForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("vendor.customer-chats.send") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    function updateUnreadCounts() {
        $.get('{{ route("vendor.customer-chats.unread-counts") }}')
    ...
    function loadMessages(customerId, shouldScroll = false, forceReload = false) {
        ...
        let url = '{{ route("vendor.customer-chats.messages", ":id") }}'.replace(':id', customerId);
            ...
            .done(function(items) {
                ...
                items.forEach(function(item) {
                    if (item.type === 'call') {
                        ...
                        const isOutgoing = call.caller_type === 'vendor';
                    ...
                    const isSent = msg.sender_type === 'vendor';
    ...
    function markAsRead(customerId) {
        $.post('{{ route("vendor.customer-chats.mark-read", ":id") }}'.replace(':id', customerId), {
            _token: '{{ csrf_token() }}'
        });
    }
    ...
    $(document).on('click', '.message-translate-btn', function(e) {
        ...
        $.ajax({
            url: '{{ route("vendor.customer-chats.translate") }}',
            type: 'POST',
    ...
    function checkForIncomingCall() {
        ...
        $.get('{{ route("vendor.customer-chats.call.offer") }}', { customer_id: currentCustomerId })
    ...
    function sendCallOffer(offer) {
        $.ajax({
            url: '{{ route("vendor.customer-chats.call.offer.send") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    function pollForCallAnswer() {
        const answerInterval = setInterval(() => {
            if (!isCallActive && peerConnection) {
                $.get('{{ route("vendor.customer-chats.call.answer.get") }}', {
                    customer_id: currentCustomerId
                })
                .done(function(response) {
                    if (response.answer) {
    ...
    function sendCallAnswer(answer, customerId) {
        $.post('{{ route("vendor.customer-chats.call.answer") }}', {
            _token: '{{ csrf_token() }}',
    ...
    function sendIceCandidate(candidate, customerId) {
        $.post('{{ route("vendor.customer-chats.call.ice.send") }}', {
            _token: '{{ csrf_token() }}',
            customer_id: customerId,
    ...
    function endCall() {
        ...
        if (customerIdToEnd) {
            $.post('{{ route("vendor.customer-chats.call.end") }}', {
                _token: '{{ csrf_token() }}',
    ...
    setInterval(function() {
        if (isCallActive && peerConnection && currentCustomerId) {
            ...
            $.get('{{ route("vendor.customer-chats.call.ice") }}', { customer_id: customerId })
                .done(function(response) {
