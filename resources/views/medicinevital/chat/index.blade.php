...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('messageForm').addEventListener('submit', function(e) {
                ...
                // User-to-user chat
                    ...
                    fetch('{{ route('medicinevital.chat.send') }}', {
                        method: 'POST',
                        headers: {
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/medicine/chat/favorites').then(res => res.json()),
                        fetch('/medicine/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/medicine/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/medicine/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/medicine/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('medicine/chat/messages') }}/${userId}`;
        ...
        function markAsRead(userId) {
            fetch(`{{ url('medicine/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
            fetch('{{ route('medicinevital.chat.unread-counts') }}')
                .then(response => response.json())
                .then(counts => {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('medicinevital.chat.send.attachment') }}';
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any medicinevitals
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
                    fetch('/medicinevital/chat/favorites')
                        .then(res => res.json())
                        .then(favorites => {
                            updateFavoriteStars(favorites);
                            showOnlyFavorites();
            ...
            $(document).on('click', '.favorite-star', function(e) {
                e.stopPropagation();
                var userId = $(this).data('userId');
                fetch('/medicinevital/chat/favorite/' + userId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
        ...
        function loadMessages(userId) {
            ...
            console.log('Starting API call for userId:', userId);
            const apiUrl = `{{ url('medicinevital/chat/messages') }}/${userId}`;
            console.log('API URL:', apiUrl);
        ...
        function markAsRead(userId) {
            fetch(`{{ url('medicinevital/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('medicinevital.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("medicinevital.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
