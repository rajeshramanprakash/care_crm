        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('lab.courier.index')}}" class="nav-link {{$uri == "courier" ? 'active' : ''}}">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('lab.query.index')}}" class="nav-link {{$uri == "query" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-shipping-fast"></i>
                        <p>Query</p>
                    </a>
                    @endif
                ...
                <li class="nav-item">
                    <a href="{{route('lab.chat.index')}}" class="nav-link {{$uri == "chat" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-comments"></i>
                        <p>
                            Chat
                            <span id="sidebar-unread-badge-lab" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#25d366; color:#fff; font-weight:bold;"></span>
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    @if($auth_user->is_ticket_enabled)
                    <a href="{{route('lab.tickets.index')}}" class="nav-link {{ strpos($route_name, 'tickets') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-ticket-alt"></i>
                        <p>
                            Tickets
                            <span id="sidebar-ticket-badge-lab" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#ffc107; color:#000; font-weight:bold;"></span>
                        </p>
                    </a>
                    @endif
                </li>
...
function updateSidebarUnreadBadgeLab() {
        fetch('{{ route('lab.chat.unread-counts') }}')
            .then(response => response.json())
            .then(counts => {
                let total = 0;
                Object.values(counts).forEach(count => total += count);
                const badge = document.getElementById('sidebar-unread-badge-lab');
                if (badge) {
                    if (total > 0) {
    ...
    updateSidebarUnreadBadgeLab();
        ...
        if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
            ...
            console.log('Sidebar real-time subscribed for updateSidebarUnreadBadgeLab');
            ...
            setInterval(updateSidebarUnreadBadgeLab, 30000);
        }
    }
    initSidebarRealtime();

    @if($auth_user->is_ticket_enabled)
    function updateSidebarTicketBadgeLab() {
        fetch('{{ route('lab.tickets.statistics') }}')
            .then(response => response.json())
            .then(stats => {
                const badge = document.getElementById('sidebar-ticket-badge-lab');
                if (badge) {
    ...
    updateSidebarTicketBadgeLab();
    setInterval(updateSidebarTicketBadgeLab, 10000);
    @endif
