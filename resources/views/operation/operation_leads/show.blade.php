                ...
                <div class="card-body">
                    <div class="container-fluid">
                                        ...
                                        <tbody>
                                            @forelse($paymentInvoices as $invoice)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('operation.operation_leads.payment_invoice.show', $invoice->id) }}" target="_blank" class="btn btn-sm btn-info" title="View Invoice">
                        ...
                        @php
                            ...
                            // Keep only entries that look like answered/missed/busy/failed calls.
                            ...
                            // Sort latest first by datetime (if available)
...
@section('footer-script')
    ...
    <script>
        ...
        function viewDeploymentDetail(id) {
            $.ajax({
                url: `{{ url('operation/operation-leads/deployment') }}/${id}/edit`,
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    if (data.success && data.deployment) {
                        ...
                        const vendorName = deployment.freelance_staff_id && deployment.freelance_staff
                            ? `${deployment.freelance_staff.name} (Freelance)`
                            : (deployment.vendor && deployment.vendor.name ? deployment.vendor.name : 'N/A');
                        const vendorRate = deployment.vendor_rate_per_day !== null && deployment.vendor_rate_per_day !== undefined
                            ? `₹${Number(deployment.vendor_rate_per_day).toFixed(2)}`
                            : 'N/A';
                        const vendorPayment = deployment.vendor_payment !== null && deployment.vendor_payment !== undefined
                            ? `₹${Number(deployment.vendor_payment).toFixed(2)}`
                            : 'N/A';
                        const verifyPayment = deployment.verify_payment ? 'Yes' : 'No';

                        const html = `
                            <div class="row">
                                <div class="col-md-6">
                                    <p><strong>Deployment Date:</strong> ${formatDateTime(deployment.deployment_date)}</p>
                                    <p><strong>From Date & Time:</strong> ${formatDateTime(deployment.deployment_from_date)}</p>
                                    <p><strong>To Date & Time:</strong> ${formatDateTime(deployment.deployment_to_date)}</p>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <p><strong>Vendor Rate Per Day:</strong> ${vendorRate}</p>
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <p><strong>Vendor Payment:</strong> ${vendorPayment}</p>
                                </div>
                                <div class="col-md-6">
                                    <p><strong>Verify Payment:</strong> ${verifyPayment}</p>
                                </div>
                        ...
                        $('#viewDeploymentDetailBody').html(html);

                        const modalElement = document.getElementById('viewDeploymentDetailModal');
                        if (modalElement) {
                            const modalInstance = bootstrap.Modal.getOrCreateInstance(modalElement);
                            modalInstance.show();
                        }
                    } else {
                        toastr.error(data.message || 'Failed to load deployment detail');
                    }
                },
                error: function(xhr) {
                    console.error('Error fetching deployment detail:', xhr);
                    toastr.error('Error loading deployment detail');
                }
            });
        }
        ...
        function openAbsentDatesModal(deploymentId) {
            ...
            fetch(`{{ route('operation.operation_leads.deployment.dates', ['id' => 'DEP_ID_PLACEHOLDER']) }}`.replace('DEP_ID_PLACEHOLDER', deploymentId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        function saveAbsentDates(deploymentId) {
            ...
            fetch(`{{ route('operation.operation_leads.deployment.save-absent', ['id' => 'DEP_ID_PLACEHOLDER']) }}`.replace('DEP_ID_PLACEHOLDER', deploymentId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
