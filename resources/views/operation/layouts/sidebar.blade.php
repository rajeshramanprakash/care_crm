...
<aside class="main-sidebar sidebar-dark-danger" style="background: white;">
    <a href="{{route('operation.dashboard')}}" class="brand-link text-center">
    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                <li class="nav-item">
                    <a href="{{route('operation.dashboard')}}" class="nav-link {{$uri == "dashboard" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-tachometer-alt"></i>
                        <p>Dashboard</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('operation.payments.index') }}" class="nav-link {{ str_starts_with($route_name, 'operation.payments.') ? 'active' : '' }}">
                        <i class="nav-icon fas fa-link"></i>
                        <p>Payments</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{route('operation.operation_leads.index')}}" class="nav-link {{ strpos($route_name, 'operation_leads') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-briefcase-medical"></i>
                        <p>Operation Leads</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{route('operation.referral_leads.index')}}" class="nav-link {{ strpos($route_name, 'referral_leads') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-share-square"></i>
                        <p>Referral Leads</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{route('operation.chat.index')}}" class="nav-link {{ strpos($route_name, 'chat') !== false && strpos($route_name, 'b2b_corporate_chat') === false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-comments"></i>
                        <p>
                            Chat
                            <span id="sidebar-unread-badge-operation" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#25d366; color:#fff; font-weight:bold;"></span>
                        </p>
                    </a>
                </li>
                ...
                @endif
                <li class="nav-item">
                    <a href="{{ route('operation.jobproc.index') }}" class="nav-link {{ strpos($route_name, 'jobproc') !== false ? 'active' : '' }}">
               ...
               <li class="nav-item">
                   <a href="{{route('operation.technical-support.index')}}" class="nav-link {{ strpos($route_name, 'technical-support') !== false ? 'active' : '' }}">
...
<script>
    ...
    function updateSidebarUnreadBadgeOperation() {
        fetch('{{ route('operation.chat.unread-counts') }}')
            .then(response => response.json())
            .then(counts => {
                let total = 0;
                Object.values(counts).forEach(count => total += count);
                const badge = document.getElementById('sidebar-unread-badge-operation');
                if (badge) {
                    if (total > 0) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        updateSidebarUnreadBadgeOperation();
        ...
        if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
            ...
            console.log('Sidebar real-time subscribed for updateSidebarUnreadBadgeOperation');
            ...
            setInterval(updateSidebarUnreadBadgeOperation, 30000);
        }
    }
    initSidebarRealtime();
    });
