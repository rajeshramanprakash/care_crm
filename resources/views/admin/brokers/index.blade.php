                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($brokerEmployeeTotal > 0)
                                                <a href="{{ route('admin.corporate-accounts.index', ['broker_id' => $item->id]) }}" class="btn btn-sm btn-outline-success py-0 px-2" title="View corporates & employees for this broker">{{ $brokerEmployeeTotal }}</a>
                                            @else
                                                <span class="text-muted">0</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-nowrap">
                                            @can('edit_broker')
                                            <a href="{{ route('admin.brokers.edit', $item) }}" class="btn btn-sm btn-info" title="Edit"><i class="fas fa-edit"></i></a>
                                            @endcan
                                            @can('delete_broker')
                                            <form action="{{ route('admin.brokers.destroy', $item) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this broker account?');">
                                                @csrf
                                                @method('DELETE')
