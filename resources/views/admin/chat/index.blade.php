            ...
            <div class="chat-card row w-100 justify-content-center align-items-stretch mobile-chat-wrapper"
                ...
                <div class="col-md-8 col-lg-8 chat-main p-0 d-flex flex-column mobile-chat-area" style="background: #fff;">
                    <div class="chat-header d-flex align-items-center px-2 py-2 border-bottom bg-white">
                        ...
                        <div class="ms-2 d-flex align-items-center gap-2 translate-lang-wrap" title="Language conversion (Gemini 2.0 Flash)">
...
function setUserUnreadCount(userId, count) {
 ...
 // { messageId: { text, lang } } - persists across re-renders

        function bindRealtimeUserChannel() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                translatedMessages = {}; // Clear translations when switching user
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/admin/chat/favorite/' + userId, {
                    method: 'POST',
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('admin/chat/messages') }}/${userId}`;
                ...
                `;
                    ...
                    `;
                        ...
                        if (message.attachment) {
                            ...
                            const transStyle = tr ? 'display:block;' : 'display:none;';
                            messageHtml = `
                        <div class="message ${isSent ? 'sent' : 'received'}" data-message-id="${message.id}">
                            ...
                            <div class="message-time">${messageDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                            ${seenHtml}
                            <div class="d-flex gap-1 mt-1" style="opacity: 0; transition: opacity 0.2s;" onmouseenter="this.style.opacity=1" onmouseleave="this.style.opacity=0">
                                <button class="message-reply-btn" data-message-id="${message.id}" data-sender-name="${escapeHtml(senderName)}" data-message-text="${escapeHtml(messageText)}" title="Reply" style="background: rgba(0,0,0,0.05); border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; color: #667781;"><i class="fa fa-reply"></i></button>
                                <button class="message-translate-btn" data-message-text="${escapeHtml(message.message || '')}" title="Translate (Gemini 2.0 Flash)" style="background: rgba(0,0,0,0.05); border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; color: #667781;"><i class="fa fa-language"></i></button>
                            </div>
                            <div class="message-translated mt-1" style="${transStyle} font-size:12px; color:#666; border-left:3px solid #F7941D; padding-left:8px;">${transHtml}</div>
                        </div>
                    `;
                        } else {
                            const tr = translatedMessages[message.id];
        ...
        function markAsRead(userId) {
            fetch(`{{ url('admin/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    ...
                    groups.forEach(group => {
                        ...
                        $('#usersList').append(
                            `<div class="chat-user-item group-item d-flex align-items-center px-3 py-2"
                                ...
                                ${isCreator ? `<button class="delete-group-btn" data-group-id="${group.id}" data-group-name="${group.name}" title="Delete Group"><i class="fa fa-trash text-danger"></i></button>` : ''}
                            </div>`
                        );
                    });
                });
        ...
        $(document).on('click', '.chat-user-item.group-item', function(e) {
            ...
            if (currentGroupId == groupId) {
                ...
                translatedMessages = {}; // Clear translations when switching group
                document.querySelectorAll('.chat-user-item').forEach(function(item) {
                    item.classList.remove('active');
                });
                            ...
                            if (messages.length === 0) {
                                ...
                                messages.forEach(msg => {
                                    ...
                                    html += `<div class="message ${isSent ? 'sent' : 'received'}" data-message-id="${msg.id}">
                                        ...
                                        <div class="d-flex gap-1 mt-1" style="opacity: 0; transition: opacity 0.2s;" onmouseenter="this.style.opacity=1" onmouseleave="this.style.opacity=0">
                                            <button class="message-reply-btn" data-message-id="${msg.id}" data-sender-name="${escapeHtmlFunc(senderName)}" data-message-text="${escapeHtmlFunc(messageText)}" title="Reply" style="background: rgba(0,0,0,0.05); border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; color: #667781;"><i class="fa fa-reply"></i></button>
                                            <button class="message-translate-btn" data-message-text="${escapeHtmlFunc(msg.message || '')}" title="Translate (Gemini 2.0 Flash)" style="background: rgba(0,0,0,0.05); border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; color: #667781;"><i class="fa fa-language"></i></button>
                                        </div>
                                        <div class="message-translated mt-1" style="${transStyle} font-size:12px; color:#666; border-left:3px solid #F7941D; padding-left:8px;">${transHtml}</div>
                                    </div>`;
                                });
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('admin.chat.send.attachment') }}';
        ...
        // Translate button click - Gemini 2.0 Flash (toggle: show on first click, hide on second)
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            if (!text || text === '[Attachment]') {
                toastr.warning('No text to translate');
                return;
            }
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
                                document.getElementById('messageInput').value = '';
        ...
        // Mobile responsive toggling logic
        ...
        // When a user or group is clicked, show chat area on mobile
        ...
        window.addEventListener('resize', function() {
            ...
            // Do NOT force show-list on mobile resize!
