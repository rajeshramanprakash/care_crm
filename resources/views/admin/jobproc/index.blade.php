...
$(function() {
    ...
    window.openFreelancerPriceChangeRequests = function (jobRequestId) {
        ...
        $.get('{{ url('admin/jobproc') }}/' + jobRequestId + '/price-change-requests')
            .done(function (res) {
    ...
    $(document).on('click', '.jp-fl-price-approve', function () {
        ...
        $.post('{{ url('admin/jobproc/price-change-requests') }}/' + id + '/approve', {
            _token: '{{ csrf_token() }}'
        }).done(function (res) {
    ...
    $(document).on('click', '#jp_fl_reject_confirm', function () {
        ...
        var $btn = $(this);
        $btn.prop('disabled', true);
        $.post('{{ url('admin/jobproc/price-change-requests') }}/' + jpFlRejectReqId + '/reject', {
            _token: '{{ csrf_token() }}',
            admin_note: note
        }).done(function (res) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res && res.success) {
                    toastr.success(res.message || 'Approved');
                    if (id) {
                        $.get('{{ url('admin/jobproc') }}/' + id, populateJobRequestProfileImageAdmin);
                    }
                } else {
