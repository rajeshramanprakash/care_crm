    ...
    <div class="dr-leegality-head">
        ...
        <div class="dr-leegality-head-actions">
            <a class="btn btn-sm btn-outline-primary"
               href="{{ route('admin.jobproc.leegality_preview_agreement', $jobRequest) }}"
            ...
            @if(!$sig || in_array($sig->signature_status, ['REJECTED', 'FAILED', 'SENT'], true))
                <button type="button"
                        ...
                        data-url="{{ route('admin.jobproc.leegality_send', $jobRequest) }}">
            ...
            <div class="dr-leegality-actions">
                @if($sig->signed_document)
                    <a class="btn btn-sm btn-outline-success"
                       href="{{ route('admin.jobproc.leegality_view_signed', [$jobRequest, $sig]) }}"
                       target="_blank" rel="noopener">
                        <i class="fas fa-eye mr-1"></i> View Signed Agreement
                    </a>
                    <a class="btn btn-sm btn-success"
                       href="{{ route('admin.jobproc.leegality_download_signed', [$jobRequest, $sig]) }}">
                        <i class="fas fa-download mr-1"></i> Download Signed PDF
                    </a>
                @endif
                @if($sig->audit_trail)
                    <a class="btn btn-sm btn-outline-primary"
                       href="{{ route('admin.jobproc.leegality_download_audit', [$jobRequest, $sig]) }}">
                ...
                @if($sig->isSignedLike() || strcasecmp((string) $sig->document_status, 'Completed') === 0)
                    <button type="button"
                            ...
                            data-url="{{ route('admin.jobproc.leegality_refresh', $jobRequest) }}">
                ...
                @if($sig->isSignedLike())
                    <button type="button"
                            ...
                            data-url="{{ route('admin.jobproc.leegality_add_my_signature', $jobRequest) }}">
