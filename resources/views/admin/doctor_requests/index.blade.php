                                            ...
                                            @foreach($drReferralList as $drv)
                                                <tr>
                                                    ...
                                                    <td class="text-right text-nowrap">
                                                        <form method="POST" action="{{ route('admin.doctor_referral_users.destroy', $drv) }}" class="d-inline" onsubmit="return confirm('Remove this referral user? Doctors linked to them will be unassigned.');">
                                                            @csrf
                                                            @method('DELETE')
...
<div class="modal fade" id="createDoctorReferralUserModal" tabindex="-1" aria-labelledby="createDoctorReferralUserModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.doctor_referral_users.store') }}">
                @csrf
...
<div class="modal fade" id="setPortalLeadCommissionModal" tabindex="-1" aria-labelledby="setPortalLeadCommissionModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.doctor_requests.portal_lead_commission') }}" id="portalLeadCommissionForm">
                @csrf
                <div class="modal-body">
...
$(function() {
    ...
    var table = $('#doctor-requests-table').DataTable({
        processing: true,
        ajax: {
            url: '{{ route('admin.doctor_requests.data') }}',
            data: function(d) {
    ...
    $('#regenerateOnlineMeetingsBtn').on('click', function() {
        ...
        $.ajax({
            url: '{{ route('admin.doctor_requests.regenerate_online_meetings') }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                scope: 'all'
    ...
    window.openDoctorViewModal = function(id) {
        ...
        $.ajax({
            url: '{{ url('admin/doctor-requests') }}/' + id + '/view-modal',
            method: 'GET',
    ...
    window.openDoctorWebsiteReviewsModal = function(id) {
        ...
        $.ajax({
            url: '{{ url('admin/doctor-requests') }}/' + id + '/website-reviews/panel',
            method: 'GET',
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        }).done(function(html) {
            $body.html(html);
    ...
    function submitDoctorStatus() {
        ...
        $.ajax({
            url: '{{ url('admin/doctor-requests') }}/' + id + '/status',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    $(document).on('click', '.dr-inline-pricing-save', function() {
        ...
        $.ajax({
            url: '{{ url('admin/doctor-requests') }}/' + id + '/pricing',
            method: 'PUT',
            data: payload,
            success: function(res) {
                if (res.success) {
    ...
    window.openDoctorPricing = function(id) {
        ...
        $.get('{{ url('admin/doctor-requests') }}/' + id + '/pricing-edit', function(res) {
        ...
        $.get('{{ url('admin/doctor-requests') }}/' + id + '/pricing-logs', function(res) {
    ...
    window.openDoctorPriceChangeRequests = function (doctorId) {
        ...
        $.get('{{ url('admin/doctor-requests') }}/' + doctorId + '/price-change-requests', function (res) {
            if (res.success) {
    ...
    $(document).on('click', '.dr-pcr-approve', function () {
        ...
        $btn.prop('disabled', true);
        $.post('{{ url('admin/doctor-requests/price-change-requests') }}/' + id + '/approve', {
            _token: '{{ csrf_token() }}'
        }, function (res) {
            if (res.success) {
    ...
    $(document).on('click', '.dr-pcr-reject', function () {
        ...
        var $btn = $(this);
        $btn.prop('disabled', true);
        $.post('{{ url('admin/doctor-requests/price-change-requests') }}/' + id + '/reject', {
            _token: '{{ csrf_token() }}',
            admin_note: note
        }, function (res) {
            if (res.success) {
