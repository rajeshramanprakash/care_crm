...
<head>
    ...
    <title>Carelix — B2B Corporate</title>
...
@php
    $portalUser = \App\Models\B2BUser::find(session('b2b_user_id'));
@endphp
<div class="wrapper">
    <nav class="main-header navbar navbar-expand navbar-light border-bottom">
        <ul class="navbar-nav"><li class="nav-item"><a class="nav-link" data-widget="pushmenu" href="#"><i class="fas fa-bars"></i></a></li></ul>
        <ul class="navbar-nav ml-auto">
            <li class="nav-item d-none d-sm-inline-block"><span class="nav-link text-muted"><span class="portal-badge">Corporate</span></span></li>
    ...
    <aside class="main-sidebar elevation-1">
        <a href="{{ route('b2b.corporate.dashboard') }}" class="brand-link text-center py-3">
        ...
        <div class="sidebar">
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column">
                    <li class="nav-item">
                        <a href="{{ route('b2b.corporate.dashboard') }}" class="nav-link {{ request()->routeIs('b2b.corporate.dashboard') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-tachometer-alt"></i><p>Dashboard</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('b2b.corporate.leads.index') }}" class="nav-link {{ request()->routeIs('b2b.corporate.leads.*') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-user-plus"></i><p>Leads</p>
                        </a>
                    </li>
                    @if($portalUser && app(\App\Services\B2BCorporateChatService::class)->corporateUserCanChat($portalUser))
                    <li class="nav-item">
                        <a href="{{ route('b2b.corporate.chat.index') }}" class="nav-link {{ request()->routeIs('b2b.corporate.chat.*') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-comments"></i><p>Chat</p>
                        </a>
                    </li>
                    @endif
                </ul>
