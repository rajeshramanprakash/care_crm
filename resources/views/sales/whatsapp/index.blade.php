...
.content-wrapper {
    ...
    .translate-lang-select { min-width: 110px; max-width: 140px; font-size: 13px; height: 32px; padding: 0.25rem 0.5rem; border-radius: 8px; border: 1px solid #e9eaeb; background-color: #fff; }
    .translate-lang-select:focus { border-color: #25d366; box-shadow: 0 0 0 0.2rem rgba(37, 211, 102, 0.2); }
    @media (max-width: 768px) { .translate-lang-wrap { order: 1; width: 100%; margin-top: 6px; } .translate-lang-select { max-width: none; } }
</style>
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    usersList.addEventListener('click', function(e) {
        ...
        loadMessages(currentNumber, 1, false);
    ...
    // Helper for getMessageHtml needed by loadMessages (extracted if not exists, but here we can reuse the logic implicitly if we create it, OR we just use the existing appendMessage logic adapted?)
    // Wait, the existing code uses `appendMessage` which builds HTML and appends.
    // `loadMessages` in newer version calls `getMessageHtml` for prepending.
    // I need `getMessageHtml` function.
    // The existing code *does not* have getMessageHtml, it has appendMessage.
    // I must create `getMessageHtml` by refactoring `appendMessage`.
    // Actually, `appendMessage` is defined later (line 414).
    // I will add `getMessageHtml` here as well based on `appendMessage` logic.
    ...
    chatMessages.addEventListener('click', function(e) {
        ...
        fetch('{{ route("sales.chat.translate") }}', {
            method: 'POST',
