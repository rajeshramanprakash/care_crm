                            ...
                            @php
                                ...
                                if (!empty($lead->recordings)) {
                                    ...
                                    $recordingsArr = is_string($lead->recording_url)
                                        ...
                                        : (is_array($lead->recording_url) ? $lead->recording_url : []);
                                }
                            @endphp
                                    ...
                                    <div class="card-body">
                                        @foreach($recordingsArr as $rec)
                                            @php
                                                $meta = [];
                                                try {
                                                    $meta = isset($rec['metadata'])
                                                        ? (is_string($rec['metadata']) ? (json_decode($rec['metadata'], true) ?? []) : (is_array($rec['metadata']) ? $rec['metadata'] : []))
                                                        : [];
                                                } catch (\Throwable $e) {
                                                    $meta = [];
                                                }
                                                    ...
                                                    <div class="mt-2">
                                                        @if(!empty($meta['datetime']))
                                                            <span class="badge badge-info" style="color:#000;">
                                                                <i class="fa fa-calendar"></i> {{ $meta['datetime'] }}
                                                            </span>
                                                        @endif
                                                        @if(!empty($meta['caller_agent']))
                                                            <span class="badge badge-secondary" style="color:#000;">
                                                                <i class="fa fa-user"></i> {{ $meta['caller_agent'] }}
                                                            </span>
                                                        @endif
                                                        @if(!empty($meta['dialstatus']))
                                                            <span class="badge badge-{{ strtolower((string)$meta['dialstatus']) === 'answered' ? 'success' : 'warning' }}" style="color:#000;">
                                                                <i class="fa fa-phone-alt"></i> {{ ucfirst((string)$meta['dialstatus']) }}
                                                            </span>
                                                        @endif
                                                    </div>
                                                </div>
