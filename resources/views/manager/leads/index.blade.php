                    ...
                    <form id="editLeadForm">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6 mb-3 p-3 bg-light rounded">
                                <label for="edit_contact_type" class="form-label"><i class="fa fa-address-book me-1"></i>
                                    Contact Type</label>
                                <select class="form-control" id="edit_contact_type" name="contact_type">
                        ...
                        <div class="row g-3">
                            <div class="col-12 mb-3 p-3 bg-light rounded" id="edit_inactive_stage_remark_group" style="display: none;">
                                ...
                                <textarea class="form-control" id="edit_inactive_stage_remark" name="inactive_stage_remark" rows="3" placeholder="Enter remark when stage is Inactive (visible to Admin)"></textarea>
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-md-6 mb-3 p-3 bg-light rounded">
                    ...
                    <form id="addLeadForm">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 mb-3 p-3 bg-light rounded" id="inactive_stage_remark_group" style="display: none;">
                                ...
                                <textarea class="form-control" id="inactive_stage_remark" name="inactive_stage_remark" rows="3" placeholder="Enter remark when stage is Inactive (visible to Admin)"></textarea>
                            </div>
                            <div class="col-md-6 mb-3">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3 p-3 bg-light rounded" id="view_query_remarks_group" style="display: none;">
                            ...
                            <div class="mb-3 p-3 bg-light rounded" id="view_status_remarks_group" style="display: none;">
    ...
    <script>
        ...
        $(function() {
            ...
            var table = $('#leads-table').DataTable({
                ...
                responsive: false,
                ajax: {
                    url: '{{ route('manager.leads.getLeads') }}',
                    data: function(d) {
            ...
            $('#updateLead').on('click', function() {
                ...
                if (data.status === 'future prospect') {
                    ...
                    var normalizedFutureProspect = normalizeDateTimeLocalForApi(data.future_prospect_date);
                    if (!normalizedFutureProspect) {
                        toastr.error('Invalid future contact date and time');
                        return;
                    }
                    data.future_prospect_date = normalizedFutureProspect;
                } else {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'follow-up' && response.follow_up_date) {
                            ...
                            $('#view_follow_up_date').text(moment(response.follow_up_date).format('DD-MM-YYYY HH:mm'));
                        } else {
            ...
            function showServiceRatesAdd() {
                ...
                // Debug logging
                console.log('Service rates request - Location:', location, 'Query:', query);

                if (location && query && query !== 'job request') {
                    // Clean the service name (remove any extra characters)
                    const cleanService = query.replace(/:.*$/, '').trim();
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            let errorMessage = 'Error loading service rates. Please try again.';
                            if (xhr.responseJSON && xhr.responseJSON.message) {
                                errorMessage = xhr.responseJSON.message;
                            } else if (xhr.status === 404) {
                                errorMessage = 'Service rates not available for this location and service combination.';
                            }
                            toastr.error(errorMessage, 'Service Rates Error');
                        }
                    });
                }
            }
            ...
            function showServiceRates() {
                ...
                // Debug logging
                console.log('Edit form service rates request - Location:', location, 'Query:', query);
