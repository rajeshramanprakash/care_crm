...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('messageForm').addEventListener('submit', function(e) {
                ...
                // User-to-user chat
                    ...
                    fetch('{{ route('dispatcher.chat.send') }}', {
                        method: 'POST',
                        headers: {
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/dispatcher/chat/favorites').then(res => res.json()),
                        fetch('/dispatcher/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/dispatcher/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/dispatcher/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/dispatcher/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
            ...
            fetch('/dispatcher/chat/favorites')
                .then(res => res.json())
                .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('dispatcher/chat/messages') }}/${userId}`;
                ...
                `;
                    ...
                    `;
                        ...
                        if (message.replied_to) {
                            ...
                            repliedHtml = `
                                <div class="replied-message" onclick="scrollToOriginalMessage(${message.reply_to_id})">
        ...
        function markAsRead(userId) {
            fetch(`{{ url('dispatcher/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('dispatcher.chat.send.attachment') }}';
                    ...
                    if (messages.length === 0) {
                        ...
                        messages.forEach((msg, index) => {
                            ...
                            if (msg.replied_to) {
                                ...
                                repliedHtml = `
                                    <div class="replied-message" onclick="scrollToOriginalMessage(${msg.reply_to_id})">
                                        <div class="replied-message-sender">${repliedSender}</div>
                                        <div class="replied-message-text">${repliedText}</div>
                                    </div>
                                `;
        ...
        function scrollToOriginalMessage(oMId) {
            const tM = document.querySelector(`.message[data-message-id="${oMId}"]`);
            if (tM) {
                ...
                const tST = sT + (tR.top - cR.top) - (cR.height / 2) + (tR.height / 2);
                cM.scrollTo({ top: tST, behavior: 'smooth' });
                setTimeout(() => {
                    tM.classList.add('message-highlight');
                    setTimeout(() => tM.classList.remove('message-highlight'), 2000);
                }, 300);
                setTimeout(() => { userHasScrolledManually = false; }, 10000);
            }
        }
        ...
        // Handle reply button clicks (delegated for dynamic content)
        document.addEventListener('click', function(event) {
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any dispatchers
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('dispatcher.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("dispatcher.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
