            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('dispatcher.dashboard') }}">Dashboard</a></li>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'all' ? 'active' : '' }}" href="{{ route('dispatcher.tickets.index', ['status' => 'all']) }}">
                                        All Tickets
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'active' ? 'active' : '' }}" href="{{ route('dispatcher.tickets.index', ['status' => 'active']) }}">
                                        Active
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'closed' ? 'active' : '' }}" href="{{ route('dispatcher.tickets.index', ['status' => 'closed']) }}">
                                        Closed
                                    </a>
                                </li>
                            </ul>
...
$(document).ready(function() {
    ...
    var table = $('#tickets-table').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: '{{ route("dispatcher.tickets.ajax") }}',
            data: function(d) {
                d.status = '{{ $status }}';
            }
        },
    ...
    function loadStatistics() {
        ...
        $.get('{{ route("dispatcher.tickets.ajax") }}', {status: '{{ $status }}'}, function(data) {
