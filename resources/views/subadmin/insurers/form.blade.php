...
<div class="content-wrapper">
    <div class="container-fluid py-3">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ $item->exists ? route('subadmin.insurers.update', $item) : route('subadmin.insurers.store') }}" enctype="multipart/form-data">
                    @csrf
                    @if($item->exists)
                        @method('PUT')
                    @endif
                    ...
                    <a href="{{ route('subadmin.insurers.index') }}" class="btn btn-secondary ml-2">Cancel</a>
                </form>
            </div>
        </div>
    </div>
