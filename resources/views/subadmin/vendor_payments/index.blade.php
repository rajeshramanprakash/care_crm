...
<div class="content-wrapper">
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                <a href="{{ route('subadmin.vendor_payments.index') }}" class="btn btn-outline-primary">
                    ...
                    <tbody>
                        ...
                        <tr class="vendor-row" data-vendor-id="{{ $vendor->id }}" data-vendor-name="{{ strtolower($vendor->name) }}" data-vendor-contact="{{ strtolower($vendor->contact_no ?? '') }}" data-vendor-email="{{ strtolower($vendor->email ?? '') }}">
                            ...
                            <td class="text-center">
                                <a href="javascript:void(0)" onclick="viewVendorStatement({{ $vendor->id }}, this.getAttribute('data-vendor-name'))" data-vendor-name="{{ e($vendor->name) }}" class="btn-vendor btn-history" title="View Statement (PDF)">
...
<div class="modal fade" id="selectLeadsModal" tabindex="-1" aria-labelledby="selectLeadsModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h6 id="selectLeadsVendorName" class="mb-3"></h6>
...
<!-- Vendor Statement Modal (PDF view + Download) -->
<div class="modal fade" id="vendorStatementModal" tabindex="-1" aria-labelledby="vendorStatementModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header" style="background: #fe992e; color: white;">
                <h5 class="modal-title" id="vendorStatementModalLabel">
                    <i class="fas fa-file-pdf"></i> Statement – <span id="vendorStatementName"></span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <iframe id="vendorStatementFrame" style="width:100%; height: 75vh; border: none;"></iframe>
            </div>
            <div class="modal-footer">
                <a id="vendorStatementDownloadBtn" href="#" target="_blank" class="btn btn-danger">
...
$(document).ready(function() {
    ...
    $('#addPaymentForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("subadmin.vendor_payments.store") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#editPaymentForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/subadmin/vendor-payments/${paymentId}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
function viewVendorHistory(vendorId, vendorName) {
    ...
    $.ajax({
        url: `/subadmin/vendor-payments/vendor/${vendorId}`,
        type: 'GET',
        success: function(response) {
            if (response.payments && response.payments.length > 0) {
                let html = '';
                response.payments.forEach(function(payment) {
                    const invoiceBtns = payment.has_invoice
                        ? `<button type="button" class="btn btn-outline-secondary btn-sm" onclick="viewInvoice(${payment.id})" title="View Invoice"><i class="fas fa-file-invoice"></i></button>
                           <a href="/subadmin/vendor-payments/${payment.id}/invoice?print=1" target="_blank" class="btn btn-outline-danger btn-sm" title="Download PDF"><i class="fas fa-file-pdf"></i></a>`
                    ...
                    html += `
                        <tr>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">${invoiceBtns}</div>
                            </td>
                            <td>
...
// Add payment for specific vendor – first open Select Leads modal
let currentAddVendorId = null;
let currentAddVendorName = null;
let selectLeadsData = null;

function addVendorPayment(vendorId, vendorName) {
    currentAddVendorId = vendorId;
    currentAddVendorName = vendorName;
    $('#selectLeadsVendorName').text(vendorName);
    ...
    fetch(`/subadmin/operation-leads/vendor-payment-details/${vendorId}`)
        .then(r => r.json())
        .then(data => {
            if (!data.success || !data.leads || data.leads.length === 0) {
                $('#selectLeadsBody').html('<tr><td colspan="6" class="text-center text-muted">No verified leads found for this vendor. You can still add payment without selecting leads.</td></tr>');
                $('#btnProceedToPayment').text('Proceed (no leads selected)').prop('disabled', false).off('click').on('click', function() {
                    proceedToPaymentForm([]);
                });
                return;
            }
            selectLeadsData = data;
            let rows = '';
            ...
            $('#btnProceedToPayment').text('Proceed to Payment Form').prop('disabled', true).off('click').on('click', function() {
                const ids = [];
                const checked = $('#selectLeadsBody .lead-checkbox:checked');
...
function proceedToPaymentForm(deploymentDetailIds) {
    ...
    $('#addPaymentVendorName').text(currentAddVendorName);
    $('#payment_vendor_id').val(currentAddVendorId);
...
function viewInvoice(paymentId) {
    document.getElementById('invoiceFrame').src = '/subadmin/vendor-payments/' + paymentId + '/invoice';
    $('#invoiceModal').modal('show');
}

// View vendor statement in modal (PDF style) with download button
function viewVendorStatement(vendorId, vendorName) {
    const statementUrl = '{{ route("subadmin.vendor_payments.statement", ["vendorId" => "__VID__"]) }}'.replace('__VID__', vendorId);
    const downloadUrl = statementUrl + '?print=1';
    document.getElementById('vendorStatementName').textContent = vendorName || 'Vendor';
    document.getElementById('vendorStatementFrame').src = statementUrl;
    document.getElementById('vendorStatementDownloadBtn').href = downloadUrl;
    $('#vendorStatementModal').modal('show');
}

// View payment details
function viewPayment(id) {
    $.get(`/subadmin/vendor-payments/${id}`, function(response) {
...
function editPayment(id) {
    ...
    $.get(`/subadmin/vendor-payments/${id}/edit`, function(response) {
...
function deletePayment(id) {
    ...
    $.ajax({
        url: `/subadmin/vendor-payments/${id}`,
        type: 'DELETE',
        headers: {
...
function refreshVendorStats(vendorId) {
    $.get(`/subadmin/vendor-payments/vendor/${vendorId}`, function(response) {
