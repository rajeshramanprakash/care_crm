...
<body>
    ...
    {{-- To (Vendor) | Document title --}}
    <div class="mid-section">
        <div class="to-block">
            <div class="to-label">To</div>
            <div class="to-line"><strong>Name:</strong> {{ $payment->vendor->name ?? 'N/A' }}</div>
            <div class="to-line"><strong>ID:</strong> {{ $payment->vendor->id ?? '—' }}</div>
            <div class="to-line"><strong>Phone:</strong> {{ $payment->vendor->contact_no ?? '—' }}</div>
        </div>
    ...
    <table class="statement-table">
        ...
        <tbody>
            @php
                // Actual amount: show as stored, trim trailing zeros (max 4 decimals)
                $fmt = function($val) {
                    $v = (float) $val;
                    $s = rtrim(sprintf('%.4f', $v), '0');
                    return rtrim($s, '.') ?: '0';
                };
                // Balance/remaining: round to 2 decimals so 111798.27 - 15833.28 = 95964.99 exactly
                $fmtBalance = function($val) {
                    return number_format(round((float) $val, 2), 2, '.', '');
                };
                $balanceDisplay = $fmtBalance($balanceAmount ?? 0);
