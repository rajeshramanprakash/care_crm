@php
    /** @var \App\Models\JobRequest $jobRequest */
    /** @var \App\Models\FreelancerLeegalitySignature|null $leegalitySignature */
    $sig = $leegalitySignature ?? null;
@endphp
<div class="dr-leegality-panel" id="frlLeegalityPanel" data-job-request-id="{{ $jobRequest->id }}">
    <div class="dr-leegality-head">
        <div>
            <div class="dr-leegality-title"><i class="fas fa-file-signature mr-1"></i> Freelancer Agreement (Leegality)</div>
            <div class="dr-leegality-sub">Send agreement to freelancer email for eSign. Status updates via webhook.</div>
        </div>
        <div class="dr-leegality-head-actions">
            <a class="btn btn-sm btn-outline-primary"
               href="{{ route('subadmin.jobproc.leegality_preview_agreement', $jobRequest) }}"
            ...
            @if(!$sig || in_array($sig->signature_status, ['REJECTED', 'FAILED', 'SENT'], true))
                <button type="button"
                        class="btn btn-sm btn-warning frl-leegality-send-btn"
                        data-url="{{ route('subadmin.jobproc.leegality_send', $jobRequest) }}">
    ...
    @if($sig)
        <div class="dr-leegality-status-card status-{{ strtolower($sig->signature_status) }}">
            ...
            <div class="dr-leegality-meta-grid">
                ...
                <div>
                    <div class="dr-leegality-meta-label">Freelancer email</div>
                    <div class="dr-leegality-meta-value">{{ $sig->signer_email ?: ($jobRequest->email ?: '—') }}</div>
                </div>
            </div>
            ...
            <div class="dr-leegality-actions">
                @if($sig->signed_document)
                    <a class="btn btn-sm btn-outline-success"
                       href="{{ route('subadmin.jobproc.leegality_view_signed', [$jobRequest, $sig]) }}"
                       target="_blank" rel="noopener">
                        <i class="fas fa-eye mr-1"></i> View Signed Agreement
                    </a>
                    <a class="btn btn-sm btn-success"
                       href="{{ route('subadmin.jobproc.leegality_download_signed', [$jobRequest, $sig]) }}">
                        <i class="fas fa-download mr-1"></i> Download Signed PDF
                    </a>
                @endif
                @if($sig->audit_trail)
                    <a class="btn btn-sm btn-outline-primary"
                       href="{{ route('subadmin.jobproc.leegality_download_audit', [$jobRequest, $sig]) }}">
                ...
                @if($sig->isSignedLike() || strcasecmp((string) $sig->document_status, 'Completed') === 0)
                    <button type="button"
                            class="btn btn-sm btn-outline-secondary frl-leegality-refresh-btn"
                            data-url="{{ route('subadmin.jobproc.leegality_refresh', $jobRequest) }}">
                        <i class="fas fa-sync-alt mr-1"></i> Refresh files
                    </button>
                @endif
                @if($sig->isSignedLike())
                    <button type="button"
                            class="btn btn-sm btn-primary frl-leegality-add-sig-btn"
                            data-url="{{ route('subadmin.jobproc.leegality_add_my_signature', $jobRequest) }}">
    ...
    @else
        <div class="dr-leegality-empty">
            No agreement sent yet. Click <strong>Send for Signature</strong> to email the freelancer via Leegality.
            @if(!trim((string) $jobRequest->email))
                <div class="text-danger mt-2"><i class="fas fa-exclamation-circle mr-1"></i> Freelancer email is missing on this registration.</div>
            @endif
        </div>
    @endif
</div>
