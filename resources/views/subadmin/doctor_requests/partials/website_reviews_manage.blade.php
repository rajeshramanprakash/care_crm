...
<div class="dr-website-reviews-manage p-2">
    ...
    @forelse ($reviews as $r)
        <div class="border rounded p-2 mb-2 d-flex justify-content-between align-items-start bg-light">
            ...
            <button type="button" class="btn btn-sm btn-outline-danger flex-shrink-0 dr-wr-delete" data-url="{{ route('subadmin.doctor_requests.website_reviews_destroy', ['doctor_request' => $doctor->id, 'review' => $r->id]) }}">Delete</button>
        </div>
    @empty
    ...
    <form id="drWebsiteReviewAddForm" class="dr-wr-add-form" enctype="multipart/form-data" data-store-url="{{ route('subadmin.doctor_requests.website_reviews_store', $doctor) }}">
        @csrf
