    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                <li class="nav-item">
                    <a href="{{route('subadmin.dashboard')}}" class="nav-link {{$uri == "dashboard" ? 'active' : ''}}">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('subadmin.courier.index')}}" class="nav-link {{$uri == "courier" ? 'active' : ''}}">
                ...
                <li class="nav-item">
                    ...
                    <a href="{{route('subadmin.query.index')}}" class="nav-link {{$uri == "query" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-shipping-fast"></i>
                        <p>Query</p>
                    </a>
                    @endif
                ...
                <li class="nav-item">
                    <a href="{{route('subadmin.chat.index')}}" class="nav-link {{$uri == "chat" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-comments"></i>
                        <p>
                            Chat
                            <span id="sidebar-unread-badge-subadmin" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#25d366; color:#fff; font-weight:bold;"></span>
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    @if($auth_user->is_ticket_enabled)
                    <a href="{{route('subadmin.tickets.index')}}" class="nav-link {{ strpos($route_name, 'tickets') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-ticket-alt"></i>
                        <p>
                            Tickets
                            <span id="sidebar-ticket-badge-subadmin" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#ffc107; color:#000; font-weight:bold;"></span>
                        </p>
                    </a>
                    @endif
                </li>
...
function updateSidebarUnreadBadgeSubAdmin() {
        fetch('{{ route('subadmin.chat.unread-counts') }}')
            .then(response => response.json())
            .then(counts => {
                let total = 0;
                Object.values(counts).forEach(count => total += count);
                const badge = document.getElementById('sidebar-unread-badge-subadmin');
                if (badge) {
                    if (total > 0) {
    ...
    updateSidebarUnreadBadgeSubAdmin();
        ...
        if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
            ...
            console.log('Sidebar real-time subscribed for updateSidebarUnreadBadgeSubAdmin');
            ...
            setInterval(updateSidebarUnreadBadgeSubAdmin, 30000);
        }
    }
    initSidebarRealtime();

    @if($auth_user->is_ticket_enabled)
    function updateSidebarTicketBadgeSubAdmin() {
        fetch('{{ route('subadmin.tickets.statistics') }}')
            .then(response => response.json())
            .then(stats => {
                const badge = document.getElementById('sidebar-ticket-badge-subadmin');
                if (badge) {
    ...
    updateSidebarTicketBadgeSubAdmin();
    setInterval(updateSidebarTicketBadgeSubAdmin, 10000);
    @endif
