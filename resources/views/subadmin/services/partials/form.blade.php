...
<form action="{{ $formAction }}" method="POST" class="dcs-form" enctype="multipart/form-data" novalidate>
    ...
    <div class="dcs-form-wrap">
            ...
            <div class="dcs-panel-body">
                <div class="form-group mb-4">
                    ...
                    <input type="text"
                           ...
                           required
                           maxlength="255"
                           placeholder="e.g. Nursing care at home"
                           autocomplete="off">
                    @error('name')
                ...
                <div class="form-group mb-4">
                    <label class="dcs-label" for="description">Description</label>
                    <textarea class="form-control @error('description') is-invalid @enderror"
                              id="description"
                              name="description"
                              rows="3"
                              maxlength="5000"
                              placeholder="Optional short description">{{ old('description', $isEdit ? ($item->description ?? '') : '') }}</textarea>
                    @error('description')
                        <span class="invalid-feedback d-block"><strong>{{ $message }}</strong></span>
                    @enderror
                </div>
                ...
                <div class="form-group mb-4">
                    ...
                    @error('icon')
                        <span class="invalid-feedback d-block"><strong>{{ $message }}</strong></span>
                    @enderror
                    <p class="dcs-hint mb-0">PNG, JPG or WebP (recommended ~120×120).</p>
                </div>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group mb-0">
                            ...
                            <input type="number"
                                   ...
                                   id="sort_order"
                                   name="sort_order"
                                   value="{{ old('sort_order', $isEdit ? ($item->sort_order ?? 0) : 0) }}"
                                   min="0"
                                   placeholder="0">
                            @error('sort_order')
                                <span class="invalid-feedback d-block"><strong>{{ $message }}</strong></span>
                            @enderror
                            <p class="dcs-hint mb-0">Lower number appears first in lists.</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="dcs-actions mt-3">
            ...
            <a href="{{ route('subadmin.services.index') }}" class="btn btn-dcs-ghost">Cancel</a>
        </div>
    </div>
</form>
