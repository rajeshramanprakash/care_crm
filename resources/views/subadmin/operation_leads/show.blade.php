                                    ...
                                    <div class="col-lg-4 col-md-5 clsr-sidebar-col d-flex flex-column gap-2">
                                        @if(!empty($operationStatusRemarks))
                                            @include('partials.crm-lead-sales-status-remarks', [
                                                ...
                                                'sidebar' => true,
                                                'inColumn' => true,
                                                'panelTitle' => 'Operation / Manager Status',
                                            ])
                                        @endif
                                        ...
                                        <tbody>
                                            @forelse($paymentInvoices as $invoice)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('subadmin.operation_leads.payment_invoice.show', $invoice->id) }}" target="_blank" class="btn btn-sm btn-info" title="View Invoice">
...
@section('footer-script')
    ...
    <script>
            ...
            $.get('{{ url("admin/operation-leads") }}/' + leadId + '/edit', function(lead) {
                ...
                for (const [key, value] of Object.entries(lead)) {
                    ...
                    if (key === 'status') {
                        ...
                        var fuValShow = formatAdminOpFpForInput(lead.follow_up_date);
                        fields +=
                        ...
                        var fpValShow = formatAdminOpFpForInput(lead.future_prospect_date);
                        fields +=
        ...
        $('#addReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/subadmin/operation-leads/invoice/${invoiceId}/received-payment`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function editReceivedPayment(id) {
            $.ajax({
                url: `/subadmin/operation-leads/received-payment/${id}/edit`,
                type: 'GET',
                success: function(response) {
                    if (response.success) {
        ...
        $('#editReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/subadmin/operation-leads/received-payment/${paymentId}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function deleteReceivedPayment(id) {
            ...
            $.ajax({
                url: `/subadmin/operation-leads/received-payment/${id}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        document.getElementById('addDeploymentDetailForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route('subadmin.operation_leads.deployment.store', $lead->id) }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        function viewDeploymentDetail(id) {
            fetch(`{{ url('admin/operation-leads/deployment') }}/${id}/edit`)
                ...
                .then(data => {
                    if (data.success) {
                        const deployment = data.deployment;
                        const vendorRate = parseFloat(deployment.vendor_rate_per_day || 0);
                        const vendorPayment = parseFloat(deployment.vendor_payment || 0);
                        ...
                        document.getElementById('viewDeploymentDetailBody').innerHTML = `
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <p><strong>Vendor Rate Per Day:</strong> ₹${vendorRate.toFixed(2)}</p>
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-12">
                                    <p><strong>Vendor Payment:</strong> ₹${vendorPayment.toFixed(2)}</p>
        ...
        function openAbsentDatesModal(deploymentId) {
            ...
            fetch(`{{ route('subadmin.operation_leads.deployment.dates', ['id' => 'DEP_ID_PLACEHOLDER']) }}`.replace('DEP_ID_PLACEHOLDER', deploymentId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        function saveAbsentDates(deploymentId) {
            ...
            fetch(`{{ route('subadmin.operation_leads.deployment.save-absent', ['id' => 'DEP_ID_PLACEHOLDER']) }}`.replace('DEP_ID_PLACEHOLDER', deploymentId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
        ...
        function formatAdminOpFpForInput(v) {
            if (!v) return '';
            var d = new Date(v);
