...
<style>
    /* Professional Status Badges for Operation Leads */
    .status-badge {
        border-radius: 8px;
        font-size: 11px;
        font-weight: 600;
                ...
                <div class="accordion-item">
                    <div class="accordion text-sm" id="accordionExample">
                                ...
                                <div class="accordion-body pl-2 pb-4">
                                    ...
                                    <div class="custom-control custom-checkbox my-1">
                                        <input class="custom-control-input" type="checkbox"
                                            ...
                                            value="{{ $vendor->id }}" {{ isset($filter_params['vendor']) &&
                                            in_array($vendor->id, $filter_params['vendor']) ? 'checked' : '' }}>
                                        <label for="team_member_{{ $vendor->id }}" class="custom-control-label">{{
                                            $vendor->name }}</label>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="btn btn-block btn-sm btn-secondary text-left text-bold text-light"
                                    type="button" data-bs-toggle="collapse" data-bs-target="#collapseongoing_stopped"
                                    aria-expanded="true" aria-controls="collapseongoing_stopped">Ongoing/Stopped</button>
                            </h2>
                            <div id="collapseongoing_stopped" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                                <div class="accordion-body pl-2 pb-4">
                                    @foreach (["stopped", "ongoing"] as $ongoing_stopped)
                                    <div class="custom-control custom-checkbox my-1">
                                        <input class="custom-control-input" type="checkbox"
                                            id="filter_ongoing_stopped_{{ str_replace(' ', '_', $ongoing_stopped) }}" name="ongoing_stopped[]"
                                            value="{{ $ongoing_stopped }}" {{ isset($filter_params['ongoing_stopped']) && in_array($ongoing_stopped, $filter_params['ongoing_stopped']) ? 'checked' : '' }}>
                                        <label for="filter_ongoing_stopped_{{ str_replace(' ', '_', $ongoing_stopped) }}" class="custom-control-label">{{ ucfirst($ongoing_stopped) }}</label>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <div class="form-group mb-3 d-none">
                                ...
                                <input type="text" class="form-control" id="executive" name="executive" value="{{ Auth::user()->id }}" readonly>
                            </div>
                            <div class="form-group mb-3">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control location-select" id="location" name="location" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-group mb-3 d-none" id="future_prospect_date_group">
                            ...
                            <div class="form-group mb-3 d-none" id="follow_up_date_group">
...
$(function() {
    ...
    var table = $('#operation-leads-table').DataTable({
        ...
        serverSide: true,
        ajax: {
                    url: '{{ route('subadmin.operation_leads.getLeads') }}',
                    data: function(d) {
        ...
        columns: [
            { data: 'lead_id', name: 'lead_id', className: 'lead-id-cell' },
            {
                data: 'date_time',
                name: 'date_time',
            ...
            { data: 'executive_name', name: 'executive_name', className: 'executive-cell' },
            { data: 'customer_name', name: 'customer_name', className: 'customer-cell', render: function(data, type, row) {
                if (row.patient_name) {
                    ...
                    return `
                        <div class="text-center">
                            ...
                            <div class="d-flex justify-content-center gap-2">
                                <button onclick="makeCall('${data}')" class="btn btn-sm call-btn" title="Call" style="color: white; padding: 6px 10px; border-radius: 6px; border: none; box-shadow: 0 1px 3px rgba(40, 167, 69, 0.3); transition: all 0.2s ease; display: flex; align-items: center; justify-content: center; width: 36px; height: 36px;">
                                    <i class="fas fa-phone" style="font-size: 12px;"></i>
                                </button>
                                <button onclick="handle_whatsapp_msg('${data}')" class="btn btn-sm whatsapp-btn" style="color: white; padding: 6px 10px; border-radius: 6px; border: none; box-shadow: 0 1px 3px rgba(37, 211, 102, 0.3); transition: all 0.2s ease; display: flex; align-items: center; justify-content: center; width: 36px; height: 36px;" title="WhatsApp">
            ...
            { data: 'location', name: 'location', className: 'location-cell' },
            { data: 'vendor_name', name: 'vendor_name', className: 'vendor-cell' },
            {
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    // Fallback: derive status from latest call logs if field is empty
                    if (!status) {
                        let combined = [];
                        const parseLogs = function(value) {
                            if (!value) return [];
                        ...
                        combined = combined.concat(parseLogs(row.recording_url));
                        combined = combined.concat(parseLogs(row.active_deployment_recording_url));

                        if (combined.length > 0) {
                            const latest = combined[combined.length - 1];
                            let meta = {};
                            try {
                                meta = typeof latest.metadata === 'string'
                                    ? JSON.parse(latest.metadata)
                                    : (latest.metadata || {});
                            } catch (e) {
                                meta = {};
                            }
                            status = latest.dialstatus || meta.dialstatus || latest.last_call_status || null;
                        }
                    }
                    ...
                    const display = (raw === 'noanswer' || raw === 'no answer') ? 'Missed' : status;
                    return `<span style="color: #000 !important; font-weight: 500;">${display}</span>`;
                }
            },
            { data: 'ongoing_stopped', name: 'ongoing_stopped', className: 'ongoing-stopped-cell' },
            {
                data: 'action',
                name: 'action',
                ...
                render: function(data, type, row) {
                    return `
                        <div class="btn-group" role="group">
                            <a href="/subadmin/operation-leads/${row.id}" class="btn action-btn view-btn" data-id="${row.id}" title="View">
                                <i class="fas fa-eye"></i>
                            </a>
                            <button class="btn action-btn edit-btn" data-id="${row.id}" title="Edit">
                                <i class="fas fa-pen"></i>
                            </button>
                            <button class="btn action-btn delete-btn" data-id="${row.id}" title="Delete">
    ...
    $('#saveOperationLead').on('click', function() {
        ...
        $.ajax({
            url: '{{ route("admin.operation_leads.store") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        $.get('/subadmin/operation-leads/' + leadId + '/edit', function(lead) {
            console.log('Edit data received:', lead);

            // Define editable fields for coordinate role
            const editableFields = [
                'query', 'query_remark', 'status', 'follow_up_date', 'future_prospect_date', 'shift_type', 'price_issue_remark', 'inactive_remark', 'closed_remark', 'patient_name', 'age', 'closed_rate',
                'vendor_name',
            ];
            ...
            for (const [key, value] of Object.entries(lead)) {
                ...
                } else if(key === 'future_prospect_date') {
                    var fpVal = '';
                    if (value) {
                        fpVal = moment(value).format('YYYY-MM-DDTHH:mm');
                    }
                    fields += `<div class='col-md-6 mb-3 d-none' id='edit_future_prospect_date_group'><label class='form-label'>Future contact date & time</label><input type='datetime-local' class='form-control' name='future_prospect_date' step='60' value='${fpVal}'></div>`;
                } else if(key === 'follow_up_date') {
                    var fuVal = '';
                    if (value) {
                        fuVal = moment(value).format('YYYY-MM-DDTHH:mm');
                    }
                    fields += `<div class='col-md-6 mb-3 d-none' id='edit_follow_up_date_group'><label class='form-label'>Follow-up date & time</label><input type='datetime-local' class='form-control' name='follow_up_date' step='60' value='${fuVal}'></div>`;
            ...
            // Add readonly fields for reference and hidden required fields
            fields = `
                <div class='col-12 mb-3'>
                    <div class='alert alert-info'>
                        ...
                        ${lead.status === 'future prospect' ? ` | <strong>Future contact date & time:</strong> ${lead.future_prospect_date ? moment(lead.future_prospect_date).format('DD-MM-YYYY HH:mm') : 'N/A'}` : ''}
                    </div>
                </div>
                ${lead.related_lead && lead.related_lead.query_remarks ? `
                <div class='col-12 mb-3'>
                    <div class='alert alert-warning'>
                        <strong><i class="fa fa-comment me-1"></i> Sales/Manager Query Remarks:</strong><br>
                        <em>${lead.related_lead.query_remarks}</em>
                    </div>
                </div>
                ` : ''}
                <!-- Hidden required fields for validation -->
                <input type='hidden' name='executive' value='${lead.executive || ''}'>
                <input type='hidden' name='customer_name' value='${lead.customer_name || ''}'>
                <input type='hidden' name='contact_no' value='${lead.contact_no || ''}'>
                <input type='hidden' name='date_time' value='${lead.date_time || ''}'>
    ...
    $('#updateOperationLead').on('click', function() {
        ...
        $.ajax({
            url: '/subadmin/operation-leads/' + leadId,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function(xhr) {
                let errorMessage = 'Could not update lead.';
                if (xhr.responseJSON) {
...
function toggleStatusRemark() {
    ...
    var fpGroup = document.getElementById('future_prospect_date_group');
    var fpInput = document.getElementById('future_prospect_date');
    var fuGroup = document.getElementById('follow_up_date_group');
    var fuInput = document.getElementById('follow_up_date');
    if (fpGroup) {
...
$(document).on('click', '.vendor-count-btn', function() {
    ...
    $.ajax({
        url: '/subadmin/operation-leads/vendor-details',
        type: 'GET',
        data: {
            lead_id: leadId,
            location: location,
...
function updateFreelancerStatus(freelancerId, status) {
    ...
    $.ajax({
        url: '/subadmin/operation-leads/update-freelancer-status',
        type: 'POST',
        data: {
...
function updateVendorCountInTable() {
    ...
    $('.vendor-count-btn').each(function() {
        ...
        $.ajax({
            url: '/subadmin/operation-leads/updated-vendor-count',
            type: 'GET',
            data: {
                lead_id: leadId,
                location: location,
