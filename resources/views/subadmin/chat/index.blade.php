                    ...
                    <div class="chat-main-inner d-flex flex-column" style="height:100%;">
                        ...
                        <div id="replyPreview" class="reply-preview-box" style="display:none;">
                            <div class="d-flex align-items-center justify-content-between px-3 py-2 bg-light border-top">
                                ...
                                <button type="button" class="btn btn-sm btn-link text-danger" id="cancelReply"><i class="fa fa-times"></i></button>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('messageForm').addEventListener('submit', function(e) {
                ...
                // Group chat
                if (currentGroupId) {
                    const rTI = document.getElementById('replyToMessageId').value;
                    const rB = { message };
                    if (rTI) { rB.reply_to_id = rTI; }
                ...
                // User-to-user chat
                    ...
                    const rTI = document.getElementById('replyToMessageId').value;
                    let bD = `receiver_id=${encodeURIComponent(receiverId)}&message=${encodeURIComponent(message)}`;
                    if (rTI) { bD += `&reply_to_id=${encodeURIComponent(rTI)}`; }
                    fetch('{{ route('subadmin.chat.send') }}', {
                        method: 'POST',
                        headers: {
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/subadmin/chat/favorites').then(res => res.json()),
                        fetch('/subadmin/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/subadmin/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/subadmin/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/subadmin/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('subadmin/chat/messages') }}/${userId}`;
                ...
                `;
                    ...
                    `;
                        ...
                        if (message.replied_to) {
                            const rT = message.replied_to.message || '[Attachment]';
                            const rS = message.replied_to.sender ? message.replied_to.sender.f_name : 'Unknown';
                            repliedHtml = `<div class="replied-message" onclick="scrollToOriginalMessage(${message.reply_to_id})"><div class="replied-message-sender">${rS}</div><div class="replied-message-text">${rT}</div></div>`;
                        }
                        let messageHtml = '';
                        if (message.attachment) {
                            ...
                            const sN = isSent ? 'You' : chatHeader.textContent;
                            messageHtml = `
                        <div class="message ${isSent ? 'sent' : 'received'}" data-message-id="${message.id}">
                            ...
                            ${seenHtml}
                            <button class="message-reply-btn" data-message-id="${message.id}" data-sender-name="${sN}" data-message-text="[Attachment]"><i class="fa fa-reply"></i> Reply</button>
                        </div>
                    `;
                        } else {
                            const sN = isSent ? 'You' : chatHeader.textContent;
                        ...
                        <div class=\"message ${isSent ? 'sent' : 'received'}\" data-message-id=\"${message.id}\">
                            ...
                            <div class=\"message-time\">${messageDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                            ${seenHtml}
                            <button class=\"message-reply-btn\" data-message-id=\"${message.id}\" data-sender-name=\"${sN}\" data-message-text=\"${message.message}\"><i class=\"fa fa-reply\"></i> Reply</button>
                        </div>
                    `;
                        }
        ...
        function markAsRead(userId) {
            fetch(`{{ url('subadmin/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
            fetch('{{ route('subadmin.chat.unread-counts') }}')
                .then(response => response.json())
                .then(counts => {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('subadmin.chat.send.attachment') }}';
                ...
                .then(messages => {
                    ...
                    if (messages.length === 0) {
                        ...
                        messages.forEach((msg, index) => {
                            ...
                            let repliedHtml = '';
                            if (msg.replied_to) {
                                const rT = msg.replied_to.message || '[Attachment]';
                                const rS = msg.replied_to.sender ? msg.replied_to.sender.f_name : 'Unknown';
                                repliedHtml = `<div class="replied-message" onclick="scrollToOriginalMessage(${msg.reply_to_id})"><div class="replied-message-sender">${rS}</div><div class="replied-message-text">${rT}</div></div>`;
                            }
                            let messageContent = '';
                            if (msg.attachment) {
        ...
        function setReplyTo(mid,sn,mt){replyToMessage={id:mid,sender:sn,text:mt};document.getElementById('replyToMessageId').value=mid;document.getElementById('replyToUser').textContent=sn;document.getElementById('replyToMessage').textContent=mt||'[Attachment]';document.getElementById('replyPreview').style.display='block';document.getElementById('messageInput').focus();}
        function cancelReply(){replyToMessage=null;document.getElementById('replyToMessageId').value='';document.getElementById('replyPreview').style.display='none';}
        ...
        document.addEventListener('DOMContentLoaded', function(){const cRB=document.getElementById('cancelReply');if(cRB){cRB.addEventListener('click',cancelReply);}});
        document.addEventListener('click',function(e){if(e.target.classList.contains('message-reply-btn')||e.target.parentElement.classList.contains('message-reply-btn')){const btn=e.target.classList.contains('message-reply-btn')?e.target:e.target.parentElement;setReplyTo(btn.getAttribute('data-message-id'),btn.getAttribute('data-sender-name'),btn.getAttribute('data-message-text'));}});
    </script>
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any subadmins
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('subadmin.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("subadmin.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
