    ...
    <script>
        $(document).ready(function() {
            ...
            function loadStatusCounts() {
                $.ajax({
                    url: '{{ route("doctor.query.status-counts") }}',
                    method: 'GET',
            ...
            serverSide: false,
            ajax: {
                url: `{{ route('doctor_query_courier_ajax') }}`,
                data: function(d) {
            ...
            columns: [
                ...
                { data: 'query_text', searchable: false, sortable: false },
                {
                    data: null,
                    render: function(data, type, row) {
        ...
        // Upload PDF logic (AJAX)
        $(document).on('click', '.btn-upload-pdf', function() {
            var form = $(this).closest('form');
            var input = form.find('input[type="file"]');
            input.click();
        });
        $(document).on('change', 'input[type="file"][name="pdf_file"]', function() {
            var form = $(this).closest('form');
            var id = form.data('id');
            var pdf_field = form.find('input[name="pdf_field"]').val();
            var file = this.files[0];
            var formData = new FormData();
            formData.append('pdf_file', file);
            formData.append('pdf_field', pdf_field);
            formData.append('_token', '{{ csrf_token() }}');
            $.ajax({
                url: `/doctor/query/${id}/update-pdf`,
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
