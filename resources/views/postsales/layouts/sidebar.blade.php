    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <li class="nav-item">
                    <a href="{{route('postsales.courier.index')}}" class="nav-link {{$uri == "cases" ? 'active' : ''}}">
                        <i class="nav-icon fa-solid fa-file"></i>
                        <p>Courier</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{route('postsales.chat.index')}}" class="nav-link {{$uri == "chat" ? 'active' : ''}}">
                        <i class="nav-icon fas fa-comments"></i>
                        <p>
                            Chat
                            <span id="sidebar-unread-badge-postsales" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#25d366; color:#fff; font-weight:bold;"></span>
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    @if($auth_user->is_ticket_enabled)
                    <a href="{{route('postsales.tickets.index')}}" class="nav-link {{ strpos($route_name, 'tickets') !== false ? 'active' : '' }}">
                        <i class="nav-icon fas fa-ticket-alt"></i>
                        <p>
                            Tickets
                            <span id="sidebar-ticket-badge-postsales" class="badge unread-count-sidebar" style="display:none; margin-left:8px; background:#ffc107; color:#000; font-weight:bold;"></span>
                        </p>
                    </a>
                    @endif
                </li>
...
function updateSidebarUnreadBadgePostSales() {
        fetch('{{ route('postsales.chat.unread-counts') }}')
            .then(response => response.json())
            .then(counts => {
                let total = 0;
                Object.values(counts).forEach(count => total += count);
                const badge = document.getElementById('sidebar-unread-badge-postsales');
                if (badge) {
                    if (total > 0) {
    ...
    updateSidebarUnreadBadgePostSales();
        ...
        if (window.ChatRealtime && window.chatRealtimeConfig && window.chatRealtimeConfig.enabled) {
            ...
            console.log('Sidebar real-time subscribed for updateSidebarUnreadBadgePostSales');
            ...
            setInterval(updateSidebarUnreadBadgePostSales, 30000);
        }
    }
    initSidebarRealtime();

    @if($auth_user->is_ticket_enabled)
    function updateSidebarTicketBadgePostSales() {
        fetch('{{ route('postsales.tickets.statistics') }}')
            .then(response => response.json())
            .then(stats => {
                const badge = document.getElementById('sidebar-ticket-badge-postsales');
                if (badge) {
    ...
    updateSidebarTicketBadgePostSales();
    setInterval(updateSidebarTicketBadgePostSales, 10000);
    @endif
