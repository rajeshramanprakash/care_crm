...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('messageForm').addEventListener('submit', function(e) {
                ...
                if (currentGroupId) {
                    ...
                    if (replyToId) { requestBody.reply_to_id = replyToId; }
                ...
                // User-to-user chat
                    ...
                    if (replyToId) { bodyData += `&reply_to_id=${encodeURIComponent(replyToId)}`; }
                    fetch('{{ route('postsales.chat.send') }}', {
                        method: 'POST',
                        headers: {
            ...
            $(document).on('click', '#showFavoritesBtn', function() {
                ...
                if (filterActive) {
                    ...
                    Promise.all([
                        fetch('/postsales/chat/favorites').then(res => res.json()),
                        fetch('/postsales/chat/group-favorites').then(res => res.json())
                    ])
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/postsales/chat/favorite/' + userId, {
                    method: 'POST',
            ...
            $(document).on('click', '.favorite-group-star', function(e) {
                ...
                fetch('/postsales/chat/group-favorite/' + groupId, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
                })
                .then(res => res.json())
                .then(data => {
                    fetch('/postsales/chat/group-favorites')
                        .then(res => res.json())
                        .then(favorites => {
            ...
            fetch('/postsales/chat/favorites')
                .then(res => res.json())
                .then(favorites => {
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('postsales/chat/messages') }}/${userId}`;
                ...
                `;
                    ...
                    `;
                        ...
                        if (message.replied_to) {
                            const rText = message.replied_to.message || '[Attachment]';
                            const rSender = message.replied_to.sender ? message.replied_to.sender.f_name : 'Unknown';
                            repliedHtml = `<div class="replied-message" onclick="scrollToOriginalMessage(${message.reply_to_id})"><div class="replied-message-sender">${rSender}</div><div class="replied-message-text">${rText}</div></div>`;
                        }
                        let messageHtml = '';
        ...
        function markAsRead(userId) {
            fetch(`{{ url('postsales/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function updateUnreadCounts() {
            fetch('{{ route('postsales.chat.unread-counts') }}')
                .then(response => response.json())
                .then(counts => {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                url = '{{ route('postsales.chat.send.attachment') }}';
        ...
        function loadGroupMessages(scrollToBottom = false) {
            ...
            fetch('/group-chat/messages/' + currentGroupId)
                ...
                .then(messages => {
                    ...
                    if (scrollToBottom && !isSearchActive) {
        ...
        function setReplyTo(mid, sn, mt) {
            replyToMessage = {id:mid,sender:sn,text:mt};
            document.getElementById('replyToMessageId').value=mid;
            document.getElementById('replyToUser').textContent=sn;
            document.getElementById('replyToMessage').textContent=mt||'[Attachment]';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const cRB = document.getElementById('cancelReply');
            if (cRB) { cRB.addEventListener('click', cancelReply); }
        });
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('message-reply-btn') || e.target.parentElement.classList.contains('message-reply-btn')) {
                const btn = e.target.classList.contains('message-reply-btn') ? e.target : e.target.parentElement;
                setReplyTo(btn.getAttribute('data-message-id'), btn.getAttribute('data-sender-name'), btn.getAttribute('data-message-text'));
            }
        });
    </script>
...
function setUserUnreadCount(userId, count) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersList2.addEventListener('click', function(e) {
                ...
                // Set currentUserId BEFORE any postsaless
                currentUserId = String(userId);
                console.log('Current user ID set to:', currentUserId);
...
function updateUnreadCounts() {
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('postsales.chat.send.attachment') }}';
            } else {
        ...
        $(document).on('click', '.message-translate-btn', function(e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.post('{{ route("postsales.chat.translate") }}', { text: rawText, target_language: targetLang, _token: '{{ csrf_token() }}' })
                .done(function(data) {
