...
class DispatcherController extends Controller
{
    public function dashboard(){
        ...
        $main_claim_cases = Cases::select('id')->where('is_post_1', 0)->where('is_post_2', 0)->where('forward_status', 1)
        ...
        $post_claim_cases = Cases::select('id')->where('is_post_1', 1)->where('is_post_2', 0)->where('forward_status', 1)
        ->where(function ($query) use ($loggedRoles) {
            $query->where('assign_member_post_role', $loggedRoles)
            ->orWhere('assign_member_role', $loggedRoles);
        })
        ...
        $post_two_claim_cases = Cases::select('id')->where('is_post_2', 1)->where('forward_status', 1)
        ->where(function ($query) use ($loggedRoles) {
            $query->where('assign_member_post_role', $loggedRoles)
            ->orWhere('assign_member_role', $loggedRoles);
        })
