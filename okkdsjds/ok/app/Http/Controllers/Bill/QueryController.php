...
class QueryController extends Controller
{
    public function index($dashboard_filters = null)
    {
        ...
        return view('bill.query.index', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
    }
...
->groupBy('cases.id')
    ...
    /**
     * Return all relevant file attachments for a case as JSON
     */
    public function files($case_id)
    {
        $case = \App\Models\Cases::findOrFail($case_id);
        $files = [
            'aadhar_attachment' => $case->aadhar_attachment,
            'aadhar_attachment_2' => $case->aadhar_attachment_2,
            'policy' => $case->policy,
            'icp_attachment' => $case->icp_attachment,
            'medicine_vitals_attached' => $case->medicine_vitals_attached,
            'pre_dispatch_pdf_attachment' => $case->pre_dispatch_pdf_attachment,
            'post_dispatch_pdf_attachment' => $case->post_dispatch_pdf_attachment,
            'post_two_dispatch_pdf_attachment' => $case->post_two_dispatch_pdf_attachment,
            'bill_attachment_1' => $case->bill_attachment_1,
            'bill_attachment_post' => $case->bill_attachment_post,
            'bill_attachment_post_two' => $case->bill_attachment_post_two,
            'discharge_summary_attachment' => $case->discharge_summary_attachment,
