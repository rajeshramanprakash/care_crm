...
class TicketController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('subadmin.tickets.index', compact('tickets', 'status'));
    }
    ...
    public function show($id)
    {
        ...
        // Mark messages as read for subadmin user
        $ticket->messages()->where('is_read', false)->update(['is_read' => true]);

        return view('subadmin.tickets.show', compact('ticket'));
    }
    ...
    public function close($id)
    {
        ...
        return redirect()->route('subadmin.tickets.index')
    ...
    public function ajaxList(Request $request)
    {
        try {
            ...
            \Log::info('SubAdmin ticket AJAX called. Status: ' . $status . ', Tickets count: ' . $tickets->count());
            ...
            $data = $tickets->map(function ($ticket) {
                try {
                    return [
                        ...
                        'actions' => view('subadmin.tickets.partials.actions', compact('ticket'))->render(),
                    ];
                } catch (\Exception $e) {
            ...
            \Log::info('SubAdmin ticket AJAX response prepared. Data count: ' . $data->count());

            return response()->json([
                'data' => $data
            ]);
        } catch (\Exception $e) {
            \Log::error('SubAdmin ticket AJAX error: ' . $e->getMessage());
            return response()->json([
                'error' => $e->getMessage(),
                'data' => []
            ], 500);
