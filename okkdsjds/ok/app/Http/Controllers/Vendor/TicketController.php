...
class TicketController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('vendor.tickets.index', compact('tickets', 'status'));
    }
    ...
    public function show($id)
    {
        ...
        // Mark role-specific messages as read for vendor
    ...
    /**
     * Submit claim (vendor action)
     */
    public function submitClaim($id)
    {
        try {
            $user = Auth::user();
            $ticket = Ticket::where('vendor_id', $user->id)
    ...
    public function ajaxList(Request $request)
    {
        try {
            ...
            \Log::info('Vendor ticket AJAX called. User ID: ' . $user->id . ', Status: ' . $status . ', Tickets count: ' . $tickets->count());
            ...
            $data = $tickets->map(function ($ticket) {
                try {
                    return [
                        ...
                        'actions' => view('vendor.tickets.partials.actions', compact('ticket'))->render(),
                    ];
                } catch (\Exception $e) {
                    \Log::error('Error processing vendor ticket ' . $ticket->id . ': ' . $e->getMessage());
                    return [
                        'id' => $ticket->id,
            ...
            \Log::info('Vendor ticket AJAX response prepared. Data count: ' . $data->count());

            return response()->json([
                'data' => $data
            ]);
        } catch (\Exception $e) {
            \Log::error('Vendor ticket AJAX error: ' . $e->getMessage());
            return response()->json([
                'error' => $e->getMessage(),
                'data' => []
            ], 500);
