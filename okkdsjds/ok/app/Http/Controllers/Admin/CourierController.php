...
class CourierController extends Controller
{
    public function index($dashboard_filters = null)
    {
        ...
        return view('admin.courier.index', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
    }
    ...
    public function ajax_list(Request $request)
    {
        ...
        $courier = Courier::select(
            ...
            ->where(function($query) use ($statusFilter) {
                ...
                } elseif ($statusFilter === 'empty_tpa') {
                    // Filter for cases with empty TPA assignments based on case type
                    $query->where(function($q) {
                        $q->where(function($subQ) {
                            // Main cases with empty TPA assignments
                            $subQ->where('courier.case_type', 'main')
                                 ->where(function($emptyQ) {
                                     $emptyQ->whereNull('cases.tpa_allot_after_claim_no_received')
                                            ->orWhere('cases.tpa_allot_after_claim_no_received', '')
                                            ->orWhere('cases.tpa_allot_after_claim_no_received', '0')
                                            ->orWhere('cases.tpa_allot_after_claim_no_received', 0);
                                 });
                        })
                        ->orWhere(function($subQ) {
                            // Post cases with empty TPA assignments
                            $subQ->where('courier.case_type', 'post')
                                 ->where(function($emptyQ) {
                                     $emptyQ->whereNull('cases.post_tpa_allot_after_claim_no_received')
                                            ->orWhere('cases.post_tpa_allot_after_claim_no_received', '')
                                            ->orWhere('cases.post_tpa_allot_after_claim_no_received', '0')
                                            ->orWhere('cases.post_tpa_allot_after_claim_no_received', 0);
                                 });
                        })
                        ->orWhere(function($subQ) {
                            // PostTwo cases with empty TPA assignments
                            $subQ->where('courier.case_type', 'postTwo')
                                 ->where(function($emptyQ) {
                                     $emptyQ->whereNull('cases.post_two_tpa_allot_after_claim_no_received')
                                            ->orWhere('cases.post_two_tpa_allot_after_claim_no_received', '')
                                            ->orWhere('cases.post_two_tpa_allot_after_claim_no_received', '0')
                                            ->orWhere('cases.post_two_tpa_allot_after_claim_no_received', 0);
                                 });
                        });
                    });
    ...
    public function update_post_one(Request $request)
    {
        ...
        if ($request->post_status == 'Query' && $case->post_status != 'Query') {
            ...
            $query->case_id = $request->case_id;
            $query->created_by = auth()->user()->id;
            $query->query = $request->query_text_post;
            $query->save();
        }
    ...
    public function update_post_two(Request $request)
    {
        ...
        if ($request->post_two_status == 'Query' && $case->post_two_status != 'Query') {
            ...
            $query->case_id = $request->case_id;
            $query->created_by = auth()->user()->id;
            $query->query = $request->query_text_post_two;
            $query->save();
        }
    ...
    public function getStatusCounts()
    {
        ...
        \Log::info('Empty claim records:', $emptyClaimRecords->toArray());

        // Count empty TPA assignments - similar logic to empty claims
        $emptyTpaQuery = Courier::select('courier.case_type', 'cases.tpa_allot_after_claim_no_received', 'cases.post_tpa_allot_after_claim_no_received', 'cases.post_two_tpa_allot_after_claim_no_received', 'cases.id')
            ->leftJoin('cases', 'courier.case_id', '=', 'cases.id')
            ->join('users', 'cases.created_by', '=', 'users.id')
            ->where(function($query) {
                $query->where(function($q) {
        ...
        $emptyTpaRecords = $emptyTpaQuery->get();
        $emptyTpaCount = $emptyTpaRecords->count();
        
        // Debug log for empty TPA
        \Log::info('Empty TPA records:', $emptyTpaRecords->toArray());
