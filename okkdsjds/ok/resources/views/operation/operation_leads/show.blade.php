            ...
            <div id="view_lead_card_container" class="card text-sm">
                ...
                <div class="card-body">
                    <div class="container-fluid">
                                        ...
                                        <tbody>
                                            @forelse($deploymentDetails as $deployment)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            $calculatedPayment = $deployment->calculated_vendor_payment;
                                                        @endphp
                                                        <strong>₹{{ number_format($calculatedPayment, 2) }}</strong>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="14" class="text-center">No deployment details found</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                        ...
                        {{-- Call Recordings from Customer (Direct Calls) --}}
                        @if ($lead->recording_url)
                            @php
                                // Get recordings from current OperationLead only
                                ...
                                // Debug: Log recordings
                                \Log::info('Operation Lead Recordings Debug:', [
                                    'lead_id' => $lead->id,
                            ...
                            <div class="card mb-5">
                                <div class="card-header text-light bg-orange">
                                    <h3 class="card-title text-light">
                                        <i class="fa fa-phone"></i> Customer Call Recordings (Direct Calls)
                                    </h3>
                                    <span class="badge badge-light float-right" style="font-size: 14px;">
                                        {{ count($recordings) }} Recording(s)
                                    </span>
                                </div>
                                <div class="card-body">
                                    @if (is_array($recordings) && count($recordings) > 0)
                                        @foreach ($recordings as $rec)
                                            @php $meta = isset($rec['metadata']) ? json_decode($rec['metadata'], true) : []; @endphp
                                            <div class="mb-3 p-3" style="background-color: #f8f9fa; border-left: 4px solid #F7941D; border-radius: 8px;">
                                                ...
                                                <div class="mt-2">
                                                    <span class="badge badge-info" style="color: #000;">
                                                        <i class="fa fa-calendar"></i> {{ isset($meta['datetime']) ? $meta['datetime'] : 'N/A' }}
                                                    </span>
                                                    <span class="badge badge-secondary" style="color: #000;">
                                                        <i class="fa fa-user"></i> {{ isset($meta['caller_agent']) ? $meta['caller_agent'] : 'N/A' }}
                                                    </span>
                                                    <span class="badge badge-{{ isset($meta['dialstatus']) && $meta['dialstatus'] == 'answered' ? 'success' : 'warning' }}" style="color: #000;">
                                                        <i class="fa fa-phone-alt"></i> {{ isset($meta['dialstatus']) ? ucfirst($meta['dialstatus']) : 'N/A' }}
                                                    </span>
                                                    @if(isset($meta['duration']))
                                                        <span class="badge badge-primary" style="color: #000;">
                                                            <i class="fa fa-clock"></i> {{ $meta['duration'] }}s
                                                        </span>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    @else
                                        <div class="text-center text-muted py-4">
                                            ...
                                            <p>No customer call recordings found.</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        {{-- Call Recordings from Active Deployment (Freelancer/Vendor Calls) --}}
                        @if ($lead->active_deployment_recording_url)
                            @php
                                $activeDepRecordings = is_array($lead->active_deployment_recording_url)
                                    ? $lead->active_deployment_recording_url
                                    : json_decode($lead->active_deployment_recording_url, true);
                                
                                $authMobile = Auth::user()->mobile;
                                $filteredActiveRecordings = array_filter($activeDepRecordings ?? [], function ($rec) use ($authMobile) {
                                    $meta = isset($rec['metadata']) ? json_decode($rec['metadata'], true) : [];
                                    return isset($meta['emp_phone']) && $meta['emp_phone'] == $authMobile;
                                });
                            @endphp
                            <div class="card mb-5">
                                <div class="card-header text-light" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%);">
                                    <h3 class="card-title text-light">
                                        <i class="fa fa-user-tie"></i> Active Deployment Call Recordings (Freelancer/Vendor)
                                    </h3>
                                    <span class="badge badge-light float-right" style="font-size: 14px;">
                                        {{ count($filteredActiveRecordings) }} Recording(s)
                                    </span>
                                </div>
                                <div class="card-body">
                                    @if (is_array($filteredActiveRecordings) && count($filteredActiveRecordings) > 0)
                                        @foreach ($filteredActiveRecordings as $rec)
                                            @php $meta = isset($rec['metadata']) ? json_decode($rec['metadata'], true) : []; @endphp
                                            <div class="mb-3 p-3" style="background-color: #f0fff4; border-left: 4px solid #28a745; border-radius: 8px;">
                                                <audio controls style="vertical-align:middle;max-width:520px;width:500px;">
                                                    <source src="{{ $rec['url'] }}" type="audio/wav">
                                                    Your browser does not support the audio element.
                                                </audio>
                                                <a href="{{ $rec['url'] }}" target="_blank"
                                                    class="btn btn-sm btn-outline-success ms-2" title="Download">
                                                    <i class="fa fa-download"></i> Download
                                                </a>
                                                <div class="mt-2">
                                                    @if(isset($meta['freelancer_name']))
                                                        <span class="badge badge-success" style="font-size: 13px;">
                                                            <i class="fa fa-user-tie"></i> {{ $meta['freelancer_name'] }}
                                                        </span>
                                                    @endif
                                                    ...
                                                    <span class="badge badge-{{ isset($meta['dialstatus']) && $meta['dialstatus'] == 'answered' ? 'success' : 'warning' }}">
                                                        <i class="fa fa-phone-alt"></i> {{ isset($meta['dialstatus']) ? ucfirst($meta['dialstatus']) : 'N/A' }}
                                                    </span>
                                                    @if(isset($meta['duration']))
                                    ...
                                    @else
                                        <div class="text-center text-muted py-4">
                                            <i class="fa fa-user-tie fa-3x mb-3" style="opacity: 0.3;"></i>
                                            <p>No active deployment call recordings found.</p>
                                            <small>Recordings from freelancers/vendors during active deployment will appear here.</small>
                                        </div>
                                    @endif
                                </div>
                            </div>
...
@section('footer-script')
    ...
    <script>
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.get('{{ url("operation/operation-leads") }}/' + leadId + '/edit', function(lead) {
                ...
                const editableFields = [
                    'date_time', 'customer_name', 'contact_no', 'address', 'location',
                    'query', 'query_remark', 'status', 'shift_type', 'price_issue_remark', 'inactive_remark', 'closed_remark', 'patient_name', 'age', 'closed_rate', 'vendor_closed_rate'
                ];
                ...
                for (const [key, value] of Object.entries(lead)) {
                    ...
                    if (key === 'status') {
                    ...
                    } else if (key === 'price_issue_remark' || key === 'inactive_remark' || key === 'closed_remark' || key === 'query_remark') {
                        ...
                        @foreach ($locations as $location)
                            locationOptions += `<option value='{{ $location->name }}' ${value==='{{ $location->name }}'?'selected':''}>{{ $location->name }}</option>`;
                        @endforeach
                        fields += `<div class='col-md-6 mb-3'><label class='form-label'>Location</label><select class='form-control' name='location' required>${locationOptions}</select></div>`;
        ...
        $('.edit-payment-btn').on('click', function() {
            var leadId = $(this).data('id');
            $.ajax({
                url: '{{ url("operation/operation-leads") }}/' + leadId + '/edit',
                type: 'GET',
                success: function(response) {
        ...
        $('#addReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/operation/operation-leads/invoice/${invoiceId}/received-payment`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function editReceivedPayment(id) {
            $.ajax({
                url: `/operation/operation-leads/received-payment/${id}/edit`,
                type: 'GET',
                success: function(response) {
                    if (response.success) {
        ...
        $('#editReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/operation/operation-leads/received-payment/${paymentId}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function deleteReceivedPayment(id) {
            ...
            $.ajax({
                url: `/operation/operation-leads/received-payment/${id}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        document.getElementById('addDeploymentDetailForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route('operation.operation_leads.deployment.store', $lead->id) }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
                ...
                .then(response => {
                    console.log('Add deployment response status:', response.status);
                    if (!response.ok) {
                ...
                .then(data => {
                    console.log('Add deployment response data:', data);
                    if (data.success) {
                        toastr.success(data.message);
                        $('#addDeploymentDetailModal').modal('hide');
                        // Clear form and reload page without query parameters
                        this.reset();
                        setTimeout(function() {
                        ...
                        toastr.error(data.message || 'Failed to add deployment detail');
                    }
                })
                .catch(error => {
                    isSubmittingDeployment = false;
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = originalText;
                    console.error('Error adding deployment detail:', error);
                    toastr.error('An error occurred while adding deployment detail: ' + error.message);
                });
        });

        function editDeploymentDetail(id) {
            console.log('Opening edit deployment modal for ID:', id);
            
            fetch(`{{ url('operation/operation-leads/deployment') }}/${id}/edit`)
                .then(response => {
                ...
                .then(data => {
                    console.log('Edit deployment data:', data);
                    if (data.success) {
                        const deployment = data.deployment;
                        const form = document.getElementById('editDeploymentDetailForm');
                        form.action = `{{ url('operation/operation-leads/deployment') }}/${id}`;
                        ...
                        console.log('Showing edit deployment modal');
                        $('#editDeploymentDetailModal').modal('show');
                    } else {
                        console.error('API returned error:', data.message);
                        toastr.error(data.message || 'Failed to load deployment details');
                    }
                })
                .catch(error => {
                    console.error('Error fetching deployment detail:', error);
                    toastr.error('An error occurred while fetching deployment detail: ' + error.message);
                });
        }
        ...
        document.getElementById('editDeploymentDetailForm').addEventListener('submit', function(e) {
            ...
            fetch(url, {
                ...
                .then(response => {
                    console.log('Update deployment response status:', response.status);
                    if (!response.ok) {
                        throw new Error(`HTTP error! status: ${response.status}`);
                    }
                    return response.json();
                ...
                .then(data => {
                    console.log('Update deployment response data:', data);
                    if (data.success) {
                        toastr.success(data.message);
                        ...
                        submitBtn.disabled = false;
                        submitBtn.innerHTML = originalText;
                        toastr.error(data.message || 'Failed to update deployment detail');
                    }
                })
                .catch(error => {
                    isUpdatingDeployment = false;
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = originalText;
                    console.error('Error updating deployment detail:', error);
                    toastr.error('An error occurred while updating deployment detail: ' + error.message);
                });
        });
        ...
        function toggleQueryRemark() {
            var querySelect = document.querySelector('select[name="query"]');
        ...
        function toggleVerifyPayment(deploymentId, isChecked) {
            $.ajax({
                url: `{{ url('operation/operation-leads/deployment') }}/${deploymentId}/toggle-verify-payment`,
                type: 'POST',
                data: {
