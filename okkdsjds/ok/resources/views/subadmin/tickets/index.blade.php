                    ...
                    <div class="card">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'all' ? 'active' : '' }}" href="{{ route('subadmin.tickets.index', ['status' => 'all']) }}">
                                        All Tickets
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'active' ? 'active' : '' }}" href="{{ route('subadmin.tickets.index', ['status' => 'active']) }}">
                                        Active
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link {{ $status === 'closed' ? 'active' : '' }}" href="{{ route('subadmin.tickets.index', ['status' => 'closed']) }}">
                                        Closed
                                    </a>
                                </li>
                            </ul>
...
$(document).ready(function() {
    ...
    var table = $('#tickets-table').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: '{{ route("subadmin.tickets.ajax") }}',
            data: function(d) {
                d.status = '{{ $status }}';
            }
        },
    ...
    function loadStatistics() {
        ...
        $.get('{{ route("subadmin.tickets.ajax") }}', {status: '{{ $status }}'}, function(data) {
