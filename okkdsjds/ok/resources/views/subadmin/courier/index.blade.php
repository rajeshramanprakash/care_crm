...
<div class="modal fade" id="editCaseModal" tabindex="-1" aria-labelledby="editCaseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCaseForm" action="{{route('subadmin.courier.case.update')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="editCasePostModal" tabindex="-1" aria-labelledby="editCasePostModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostModalForm"  action="{{route('subadmin.courier.case.update.post_one')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="editCasePostTwoModal" tabindex="-1" aria-labelledby="editCasePostTwoModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostTwoModalForm" action="{{route('subadmin.courier.case.update.post_two')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    ...
    $('#clainNoForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `{{ route('subadmin.cases.save-claimno') }}`,
            method: 'POST',
            data: formData,
    ...
    $('#tpaAllotmentForm').on('submit', function(e) {
        ...
        formData._token = '{{ csrf_token() }}';
        $.ajax({
            url: `{{ route('subadmin.cases.save-tpa') }}`,
            method: 'POST',
            data: formData,
            success: function(response) {
                alert(response.message);
    ...
    $(document).ready(function() {
        ...
        const table = $('#casesTable').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: `{{ route('subadmin_courier_ajax') }}`,
                data: function(d) {
        ...
        $(document).on('click', '.btn-view-case', function() {
            ...
            window.location.href = '/subadmin/cases/view/' + encryptedId;
        });
        ...
        $('#status').on('change', function() {
            if ($(this).val() === 'Query') {
                $('#queryTextGroup').show();
                $('#editCaseForm input[name="approved_amt"]').closest('.form-group').hide();
            } else {
                $('#queryTextGroup').hide();
                $('#editCaseForm input[name="approved_amt"]').closest('.form-group').show();
            }
        });
        ...
        $('#post_status').on('change', function() {
            if ($(this).val() === 'Query') {
                ...
                $('#editCasePostModalForm input[name="post_ammount"]').closest('.form-group').hide();
            } else {
                $('#postQueryTextGroup').hide();
                $('#editCasePostModalForm input[name="post_ammount"]').closest('.form-group').show();
            }
        });
        ...
        $('#post_two_status').on('change', function() {
            if ($(this).val() === 'Query') {
                ...
                $('#editCasePostTwoModalForm input[name="post_two_ammount"]').closest('.form-group').hide();
            } else {
                $('#postTwoQueryTextGroup').hide();
                $('#editCasePostTwoModalForm input[name="post_two_ammount"]').closest('.form-group').show();
            }
        });
        // Trigger change on modal open to set correct state
        $('#editCaseModal').on('shown.bs.modal', function () {
        ...
        function updateStatusCounts() {
            $.ajax({
                url: '{{ route("subadmin.courier.status-counts") }}',
                method: 'GET',
