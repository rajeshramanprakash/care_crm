...
@section('content')
    ...
    <script>
                    ...
                    .then(messages => {
                        ...
                        if (messages.length === 0) {
                            ...
                            messages.forEach(msg => {
                                html += `<div class=\"message ${msg.sender_id == {{ Auth::id() }} ? 'sent' : 'received'}\">\n                                <div class=\"fw-bold\">${msg.sender.f_name}</div>\n                                <div class=\"message-content\">${msg.attachment ? (['jpg','jpeg','png','gif','webp'].includes((msg.attachment.split('.').pop()||'').toLowerCase()) ? `<img src=\"/storage/${msg.attachment}\" class=\"img-fluid\" style=\"max-width:200px;max-height:200px; border-radius:8px;\" alt=\"Attachment\">` : `<a href=\"/storage/${msg.attachment}\" target=\"_blank\" class=\"btn btn-sm btn-primary\"><i class=\"fa fa-download\"></i> Download</a>`) : msg.message}</div>\n                                <div class=\"message-time\">${new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>\n                            </div>`;
                            });
                        }
        ...
        // On page load, show user list on mobile
        if (window.innerWidth <= 480) {
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelector('.mobile-chat-wrapper').classList.add('show-list');
                document.querySelector('.mobile-chat-wrapper').classList.remove('show-chat');
        ...
        // When back button is clicked, show user list
        function showUserListMobile() {
            if (window.innerWidth <= 480) {
                document.querySelector('.mobile-chat-wrapper').classList.add('show-list');
                document.querySelector('.mobile-chat-wrapper').classList.remove('show-chat');
