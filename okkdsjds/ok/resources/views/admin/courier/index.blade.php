                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-2 col-sm-4 col-6 mb-2">
                                    <div class="status-filter-box" data-status="Paid" style="background-color: #17A2B8; color: white; padding: 15px; text-align: center; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                        <strong>Paid</strong>
                                        <div class="count" id="count-paid">-</div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-sm-4 col-6 mb-2">
                                    <div class="status-filter-box" data-status="Reject" style="background-color: #DC3545; color: white; padding: 15px; text-align: center; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                        <strong>Reject</strong>
                                        <div class="count" id="count-reject">-</div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-sm-4 col-6 mb-2">
                                ...
                                <div class="col-md-2 col-sm-4 col-6 mb-2">
                                    <div class="status-filter-box" data-status="empty_tpa" style="background-color: #E83E8C; color: white; padding: 15px; text-align: center; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                        <strong>Empty TPA</strong>
                                        <div class="count" id="count-empty-tpa">-</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <form id="tpaAllotmentForm">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="case_type" id="caseTypeTpa">

                        <!-- Main Claim TPA Fields -->
                        <div id="mainTpaFields">
                        ...
                        <!-- Post One TPA Fields -->
                        <div id="postTpaFields" style="display: none;">
                        ...
                        <!-- Post Two TPA Fields -->
                        <div id="postTwoTpaFields" style="display: none;">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div id="postTwoClaimFields" style="display: none;">
                            ...
                            <div class="form-group">
                                <label for="post_two_claim_no_link">Post 2 claim link</label>
...
<div class="modal fade" id="editCaseModal" tabindex="-1" aria-labelledby="editCaseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCaseForm" action="{{route('admin.courier.case.update')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="editCasePostModal" tabindex="-1" aria-labelledby="editCasePostModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostModalForm"  action="{{route('admin.courier.case.update.post_one')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="editCasePostTwoModal" tabindex="-1" aria-labelledby="editCasePostTwoModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostTwoModalForm" action="{{route('admin.courier.case.update.post_two')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    function openTpaForm(caseId, caseType, tpaType, tpaAllotment, tpaAllotmentTwo, postTpaType, postTpaAllotment, postTpaAllotmentTwo, postTwoTpaType, postTwoTpaAllotment, postTwoTpaAllotmentTwo) {
            $('#caseId').val(caseId);
            $('#caseTypeTpa').val(caseType);
            
            // Hide all field groups first
            $('#mainTpaFields').hide();
            $('#postTpaFields').hide();
            $('#postTwoTpaFields').hide();
            ...
            // Show relevant fields based on case type
            if(caseType === 'main') {
                $('#mainTpaFields').show();
                if (tpaType) {
        ...
        function openClaimForm(caseId, caseType, claim_no, post_claim_no, post_two_claim_no, claim_no_link, post_claim_no_link, post_two_claim_no_link) {
            $('#caseIdclaim').val(caseId);
            $('#caseTypeclaim').val(caseType);
            
            // Hide all field groups first
            $('#mainClaimFields').hide();
            $('#postClaimFields').hide();
            $('#postTwoClaimFields').hide();
            
            // Clear all fields
            $('#claim_no_form_id').val('');
            $('#claim_no_link_form_id').val('');
            $('#post_claim_no_form_id').val('');
            $('#post_claim_no_link_form_id').val('');
            $('#post_two_claim_no_form_id').val('');
            $('#post_two_claim_no_link_form_id').val('');
            
            // Show relevant fields based on case type
            if(caseType === 'main') {
                $('#mainClaimFields').show();
                $('#claim_no_form_id').val(claim_no);
                $('#claim_no_link_form_id').val(claim_no_link);
            } else if(caseType === 'post') {
                $('#postClaimFields').show();
                $('#post_claim_no_form_id').val(post_claim_no);
                $('#post_claim_no_link_form_id').val(post_claim_no_link);
            } else if(caseType === 'postTwo') {
                $('#postTwoClaimFields').show();
            ...
            const table = $('#casesTable').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        data: null,
                        render: function(data, type, row) {
return `<button class="btn btn-primary btn-sm" onclick="openClaimForm('${row.id}', '${row.case_type}', '${row.claim_no || ''}', '${row.post_claim_no || ''}', '${row.post_two_claim_no || ''}', '${row.claim_no_link || ''}', '${row.post_claim_no_link || ''}', '${row.post_two_claim_no_link || ''}')">Claim no.</button>`;
                        },
                        orderable: false
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            let linkUrl = '';
                            if(row.case_type == 'main' && row.claim_no_link){
                                linkUrl = row.claim_no_link;
                            }else if(row.case_type == 'post' && row.post_claim_no_link){
                                linkUrl = row.post_claim_no_link;
                            }else if(row.case_type == 'postTwo' && row.post_two_claim_no_link){
                                linkUrl = row.post_two_claim_no_link;
                            }
                            
                            if(linkUrl){
                                return `<a href="${linkUrl}" target="_blank" class="btn btn-info btn-sm">Link</a>`;
                            }else{
                    ...
                    },
                    {
                        data: null,
                        render: function(data, type, row) {
                            return `<button class="btn btn-primary btn-sm" onclick="openTpaForm(${row.id}, '${row.case_type}', '${row.tpa_type || ''}', '${row.tpa_allot_after_claim_no_received || ''}', '${row.tpa_allot_after_claim_no_received_two || ''}',
                            '${row.post_tpa_type || ''}','${row.post_tpa_allot_after_claim_no_received || ''}','${row.post_tpa_allot_after_claim_no_received_two || ''}','${row.post_two_tpa_type || ''}','${row.post_two_tpa_allot_after_claim_no_received || ''}','${row.post_two_tpa_allot_after_claim_no_received_two || ''}')">TPA Allot</button>`;
                        },
                        orderable: false
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return '<div class="display-flex"><button class="btn btn-info btn-view-case" data-id="' +
                                ...
                                '">View</button></div> <button class="btn btn-danger btn-del-case" data-id="' +
                                row.courier_id + '">Delete</button></div>';
                        },
                        orderable: false
                    },
                ],
    ...
    function updateStatusCounts() {
        $.ajax({
            url: '{{ route("admin.courier.status-counts") }}',
            method: 'GET',
            success: function(counts) {
                ...
                if (counts['empty_tpa']) {
                    $('#count-empty-tpa').text(counts['empty_tpa']);
                }
            },
            error: function(xhr) {
