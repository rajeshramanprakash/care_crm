...
class ManagerController extends Controller
{
    ...
    {
        $page_heading = 'Sales Manager Dashboard';

        // Get today's leads count for the logged-in manager's team
        $managerId = auth()->user()->id;
        $teamMemberIds = User::where('parent_id', $managerId)->pluck('id')->toArray();
        ...
        $activeUsers = $teamMembers->where('is_active', 1)->count();
        $inactiveUsers = $teamMembers->where('is_active', 0)->count();

        // Get task statistics for team including parent manager assigned tasks
        $allUserIds = array_merge([$managerId], $teamMemberIds); // Include manager themselves

        // Get parent manager ID
        $parentManagerId = User::where('id', $managerId)->value('parent_id');
        ...
        $pendingTasks = Task::where(function($q) use ($allUserIds, $parentManagerId) {
            $q->whereIn('assigned_to', $allUserIds)
              ->where('status', 'pending');

            // Also include pending tasks assigned by parent manager to team members
            if ($parentManagerId) {
                $q->orWhere(function($subQ) use ($parentManagerId, $allUserIds) {
                    $subQ->where('assigned_by', $parentManagerId)
                         ->whereIn('assigned_to', $allUserIds)
        ...
        $completedTasks = Task::where(function($q) use ($allUserIds, $parentManagerId) {
            $q->whereIn('assigned_to', $allUserIds)
              ->where('status', 'completed');

            // Also include completed tasks assigned by parent manager to team members
            if ($parentManagerId) {
                $q->orWhere(function($subQ) use ($parentManagerId, $allUserIds) {
                    $subQ->where('assigned_by', $parentManagerId)
                         ->whereIn('assigned_to', $allUserIds)
        ...
        $overdueTasks = Task::where(function($q) use ($allUserIds, $parentManagerId) {
            $q->whereIn('assigned_to', $allUserIds)
              ->where('due_date', '<', Carbon::today())
              ->where('status', '!=', 'completed');

            // Also include overdue tasks assigned by parent manager to team members
            if ($parentManagerId) {
                $q->orWhere(function($subQ) use ($parentManagerId, $allUserIds) {
                    $subQ->where('assigned_by', $parentManagerId)
                         ->whereIn('assigned_to', $allUserIds)
        ...
        return view('manager.dashboard', compact('page_heading', 'todayLeads', 'totalTasks', 'pendingTasks', 'completedTasks', 'overdueTasks', 'locations', 'services', 'activeUsers', 'inactiveUsers', 'teamMembers'));
    }
