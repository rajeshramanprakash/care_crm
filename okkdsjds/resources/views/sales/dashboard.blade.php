    ...
    <script>
        ...
        function updateLeadsDisplay(data) {
            ...
            if (data.leads && data.leads.length > 0) {
                ...
                tableBody.innerHTML = '<tr><td colspan="9" class="text-center">No leads found for the selected period.</td></tr>';
            }
        }
                        ...
                        data.callLogs.forEach(log => {
                            ...
                            tableHTML += `
                                <tr data-log-id="${log.id}">
                                    ...
                                    <td>${log.executive ? log.executive.f_name + ' ' + log.executive.l_name : 'N/A'}</td>
                                    <td>${log.customer_name || 'N/A'}</td>
                                    <td>${callReceived}</td>
                                    <td>${log.agent_name || 'N/A'}</td>
                                    <td>${log.agent_number || 'N/A'}</td>
        ...
        // Edit lead from recent modal function
        function editLeadFromRecentModal(leadId) {
            // Redirect to the lead show/edit page
            window.location.href = `/sales/leads/${leadId}`;
        }
            ...
            data.leads.forEach(lead => {
                ...
                tableHTML += `
                    <tr>
                        ...
                        <td>
                            <a href="/sales/leads/${lead.id}" class="btn btn-sm btn-outline-primary" title="View Lead" target="_blank">
                                <i class="fas fa-eye"></i>
                            </a>
                        </td>
                    </tr>
        ...
        function updateRecentCallsDisplay(data) {
            ...
            data.calls.forEach(call => {
                ...
                const recordingButton = call.has_recording ? 
                    `<button class="btn btn-sm btn-info view-recording-btn" 
                            data-recording-url="${(call.recording_url || '').replace(/"/g, '&quot;')}" 
                            data-customer-name="${(call.customer_name || 'N/A').replace(/"/g, '&quot;')}" 
                            data-call-datetime="${(call.call_datetime || 'N/A').replace(/"/g, '&quot;')}" 
                            title="View Recording">
                        <i class="fas fa-headphones"></i> Recording
                    </button>` : 
            ...
            tableBody.querySelectorAll('.view-recording-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const recordingUrl = this.getAttribute('data-recording-url');
                    const customerName = this.getAttribute('data-customer-name');
                    const callDateTime = this.getAttribute('data-call-datetime');
                    openRecordingModal(recordingUrl, customerName, callDateTime);
                });
            });
        }
        ...
        function openRecordingModal(recordingUrl, customerName, callDateTime) {
            if (!recordingUrl) {
            ...
            // Set modal content
            document.getElementById('recordingCustomerName').textContent = customerName || 'N/A';
            document.getElementById('recordingCallDateTime').textContent = callDateTime || 'N/A';
            ...
            downloadLink.download = `recording_${customerName || 'call'}_${callDateTime || Date.now()}.mp3`;
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="fas fa-headphones"></i> Call Recording
                    </h5>
                ...
                <div class="modal-body">
                    <div style="margin-bottom: 20px;">
                        <div style="margin-bottom: 10px;">
                            <strong>Customer:</strong> <span id="recordingCustomerName">N/A</span>
                        </div>
                        <div style="margin-bottom: 10px;">
                            <strong>Call Date & Time:</strong> <span id="recordingCallDateTime">N/A</span>
                        </div>
                    </div>
                    
                    <div style="margin-bottom: 20px;">
                        <audio id="recordingAudioPlayer" controls style="width: 100%;">
                            Your browser does not support the audio element.
                        </audio>
                    </div>
                    
                    <div style="text-align: center;">
                        <a id="recordingDownloadLink" href="#" download class="btn btn-primary">
