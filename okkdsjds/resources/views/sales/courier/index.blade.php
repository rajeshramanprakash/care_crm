...
<div class="modal fade" id="editCaseModal" tabindex="-1" aria-labelledby="editCaseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCaseForm" action="{{route('sales.courier.case.update')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="editCasePostModal" tabindex="-1" aria-labelledby="editCasePostModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostModalForm"  action="{{route('sales.courier.case.update.post_one')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body row">
                    ...
                    <div class="form-group col-lg-6 col-sm-12" id="approvedAmtGroupPost">
                        <label for="post_ammount">Approved Amount</label>
                        <input type="text" class="form-control" name="post_ammount" id="post_ammount">
                    </div>
                    <div class="form-group col-lg-6 col-sm-12" id="queryTextGroupPost" style="display:none;">
                        <label for="query_text_post">Query Remark</label>
                        <textarea class="form-control" name="query_text_post" id="query_text_post" rows="3"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
<div class="modal fade" id="editCasePostTwoModal" tabindex="-1" aria-labelledby="editCasePostTwoModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form id="editCasePostTwoModalForm" action="{{route('sales.courier.case.update.post_two')}}" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body row">
                    ...
                    <div class="form-group col-lg-6 col-sm-12" id="approvedAmtGroupPostTwo">
                    ...
                    <div class="form-group col-lg-6 col-sm-12" id="queryTextGroupPostTwo" style="display:none;">
                        <label for="query_text_post_two">Query Remark</label>
                        <textarea class="form-control" name="query_text_post_two" id="query_text_post_two" rows="3"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
<script>
    ...
    $('#clainNoForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `{{ route('sales.cases.save-claimno') }}`,
            method: 'POST',
            data: formData,
    ...
    $('#tpaAllotmentForm').on('submit', function(e) {
        ...
        formData._token = '{{ csrf_token() }}';
        $.ajax({
            url: `{{ route('sales.cases.save-tpa') }}`,
            method: 'POST',
            data: formData,
                success: function(response) {
                    alert(response.message);
    ...
    $(document).ready(function() {
        ...
        function updateStatusCounts() {
            $.ajax({
                url: '{{ route("sales.courier.status-counts") }}',
                method: 'GET',
        ...
        // AJAX form submission for edit case form
        $('#editCaseForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                contentType: false,
                success: function(response) {
                    alert(response.message || 'Case updated successfully.');
        ...
        // AJAX form submission for edit case post form
        ...
        // AJAX form submission for edit case post two form
        ...
        // Toggle Approved Amount/Query textarea based on status
        ...
        // On modal open, set correct field visibility
        ...
        $('#post_status').on('change', function() {
            if ($(this).val() === 'Query') {
                $('#approvedAmtGroupPost').hide();
                $('#queryTextGroupPost').show();
            } else {
                $('#approvedAmtGroupPost').show();
                $('#queryTextGroupPost').hide();
            }
        });
        $('#editCasePostModal').on('show.bs.modal', function() {
        ...
        $('#post_two_status').on('change', function() {
            if ($(this).val() === 'Query') {
                $('#approvedAmtGroupPostTwo').hide();
                $('#queryTextGroupPostTwo').show();
            } else {
                $('#approvedAmtGroupPostTwo').show();
                $('#queryTextGroupPostTwo').hide();
            }
        });
        $('#editCasePostTwoModal').on('show.bs.modal', function() {
            var status = $('#post_two_status').val();
            if (status === 'Query') {
                $('#approvedAmtGroupPostTwo').hide();
                $('#queryTextGroupPostTwo').show();
            } else {
