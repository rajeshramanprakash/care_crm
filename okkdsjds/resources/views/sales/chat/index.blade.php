    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $(document).on('click', '.favorite-star', function(e) {
                ...
                fetch('/sales/chat/favorite/' + userId, {
                    method: 'POST',
        ...
        function loadMessages(userId) {
            ...
            const apiUrl = `{{ url('sales/chat/messages') }}/${userId}`;
                    ...
                    `;
                        ...
                        if (message.attachment) {
                            ...
                            if (message.attachment_type && message.attachment_type.startsWith('image/')) {
                                ...
                                attachmentHtml = `
                                    <div style="background:#e6f0ff; color:#000; border-radius:8px; padding:16px; display:flex; align-items:center; gap:12px;">
        ...
        function markAsRead(userId) {
            fetch(`{{ url('sales/chat/mark-read') }}/${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        document.getElementById('sendAttachmentBtn').addEventListener('click', function() {
            ...
            if (currentUserId) {
                ...
                var url = '{{ route('sales.chat.send.attachment') }}';
            } else {
        ...
        // On user click, call showChatMobile
        const usersList = document.getElementById('usersList');
        if (usersList) {
            usersList.addEventListener('click', function(e) {
                let target = e.target;
        ...
        // On page load, always start with user list on mobile
        if (window.innerWidth <= 480) {
            document.querySelector('.mobile-chat-wrapper').classList.add('show-list');
            document.querySelector('.mobile-chat-wrapper').classList.remove('show-chat');
        }
