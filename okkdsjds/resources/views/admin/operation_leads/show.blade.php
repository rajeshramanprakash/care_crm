                    ...
                    <div class="container-fluid">
                        <div class="card mb-5">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    @endif
                                    @if($crm_lead && $crm_lead->query_remarks)
                                    <div class="col-sm-6">
                                        <span class="text-bold mx-1" style="color: #F7941D">Sales/Manager Query Remarks: </span>
                                        <span class="mx-1">{{ $crm_lead->query_remarks }}</span>
                                    </div>
                                    @endif
                                    <div class="col-sm-6">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <span class="mx-1">
                                            <span class="badge badge-{{ $lead->last_call_status_display == 'answered' ? 'success' : ($lead->last_call_status_display == 'no answer' ? 'warning' : ($lead->last_call_status_display == 'busy' ? 'info' : ($lead->last_call_status_display == 'failed' ? 'danger' : ($lead->last_call_status_display == 'completed' ? 'primary' : 'secondary')))) }}">
                        ...
                        @if ($lead->recording_url)
                            @php
                                ...
                                \Log::info('Admin Lead Recordings Debug:', [
                                    'lead_id' => $lead->id,
                                        ...
                                        @foreach ($recordings as $rec)
                                            ...
                                            <div class="mb-3 p-3" style="background-color: #f8f9fa; border-left: 4px solid #F7941D; border-radius: 8px;">
                                                ...
                                                <div class="mt-2">
                                                    ...
                                                    <span class="badge badge-secondary" style="color: #fff;">
                                                        <i class="fa fa-user"></i> {{ isset($meta['caller_agent']) ? $meta['caller_agent'] : 'N/A' }}
                                                    </span>
                                                    <span class="badge badge-{{ isset($meta['dialstatus']) && $meta['dialstatus'] == 'answered' ? 'success' : 'warning' }}" style="color: #fff;">
...
@section('footer-script')
    ...
    <script>
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.get('{{ url("admin/operation-leads") }}/' + leadId + '/edit', function(lead) {
        ...
        $('#updateOperationLead').on('click', function() {
            ...
            $.ajax({
                url: '{{ url("admin/operation-leads") }}/' + leadId,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $('.edit-payment-btn').on('click', function() {
            var leadId = $(this).data('id');
            $.ajax({
                url: '{{ url("admin/operation-leads") }}/' + leadId + '/edit',
                type: 'GET',
                success: function(response) {
        ...
        $('#addReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/admin/operation-leads/invoice/${invoiceId}/received-payment`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function editReceivedPayment(id) {
            $.ajax({
                url: `/admin/operation-leads/received-payment/${id}/edit`,
                type: 'GET',
                success: function(response) {
                    if (response.success) {
        ...
        $('#editReceivedPaymentForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/admin/operation-leads/received-payment/${paymentId}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function deleteReceivedPayment(id) {
            ...
            $.ajax({
                url: `/admin/operation-leads/received-payment/${id}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        document.getElementById('addDeploymentDetailForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route('admin.operation_leads.deployment.store', $lead->id) }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        function editDeploymentDetail(id) {
            fetch(`{{ url('admin/operation-leads/deployment') }}/${id}/edit`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        const deployment = data.deployment;
                        const form = document.getElementById('editDeploymentDetailForm');
                        form.action = `{{ url('admin/operation-leads/deployment') }}/${id}`;
        ...
        function updateFreelancerStatus(freelancerId, status) {
            ...
            $.ajax({
                url: '{{ route("admin.operation_leads.update_freelancer_status") }}',
                type: 'POST',
                data: {
            ...
            if (currentModal.length && currentModal.hasClass('show')) {
                ...
                if (leadInfo.length) {
                    ...
                    if (currentVendorBtn.length) {
                        ...
                        $.ajax({
                            url: '{{ route("admin.operation_leads.vendor_details") }}',
                            type: 'GET',
                            data: {
                                lead_id: leadId,
                                location: location,
        ...
        function updateVendorCountInTable() {
            ...
            $('.vendor-count-btn').each(function() {
                ...
                $.ajax({
                    url: '{{ route("admin.operation_leads.updated_vendor_count") }}',
                    type: 'GET',
                    data: {
                        lead_id: leadId,
                        location: location,
        ...
        function toggleVerifyPayment(deploymentId, isChecked) {
            $.ajax({
                url: `{{ url('admin/operation-leads/deployment') }}/${deploymentId}/toggle-verify-payment`,
                type: 'POST',
                data: {
