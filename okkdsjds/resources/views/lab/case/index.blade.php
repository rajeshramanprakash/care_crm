    ...
    <script>
        ...
        return `<input type="checkbox" class="working-checkbox" data-id="${row.id}" ${row.is_working_row ? 'checked' : ''}> ${data}`;
                    ...
                    },
                    {
                        data: 'doa',
                        render: function(data, type, row) {
                            return `${moment(row.doa).format("DD-MMM-YYYY")} at ${moment(row.doa_time, "HH:mm:ss").format("hh:mm A")}`;
                        },
                        searchable: true,
                        sortable: false
                    },
                    {
                        data: 'dod',
                        render: function(data, type, row) {
                                                        return `${moment(row.dod).format("DD-MMM-YYYY")} at ${moment(row.dod_time, "HH:mm:ss").format("hh:mm A")}`;
                        },
                        searchable: true,
                        sortable: false
                    },
            ...
            $(document).on('click', '.btn-view-case', function() {
                ...
                window.location.href = '/lab/cases/view/' + encryptedId; // Redirect to case show page
            });
        });
