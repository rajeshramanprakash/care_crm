...
$(function() {
    var table = $('#job-request-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route('operation-manager.jobproc.jobrequests') }}',
            data: function(d) {
    ...
    $(document).on('click', '.show-job', function() {
        var id = $(this).data('id');
        $.get('{{ url('operation-manager/jobproc') }}/' + id, function(data) {
    ...
    $(document).on('click', '.edit-job', function() {
        var id = $(this).data('id');
        $.get('{{ url('operation-manager/jobproc') }}/' + id + '/edit', function(data) {
            $('#edit_id').val(data.id);
    ...
    $('#saveJobRequest').on('click', function() {
        ...
        $.ajax({
            url: '{{ route('operation-manager.jobproc.store') }}',
            type: 'POST',
            data: formData,
            success: function(response) {
                console.log('Success response:', response);
