                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-2 col-sm-4 col-6 mb-2">
                                        <div class="status-filter-box" data-status="all"
                                            style="background-color: #007BFF; color: white; padding: 15px; text-align: center; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                            <strong>All Queries</strong>
                                            <div class="count" id="count-all">-</div>
                                        </div>
                                    </div>
                                    <div class="col-md-2 col-sm-4 col-6 mb-2">
                                        <div class="status-filter-box" data-status="empty_claim"
                                            style="background-color: #6F42C1; color: white; padding: 15px; text-align: center; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadStatusCounts() {
                $.ajax({
                    url: '{{ route("bill.query.status-counts") }}',
                    method: 'GET',
            ...
            const table = $('#casesTable').DataTable({
                ...
                serverSide: false,
                ajax: {
                    url: `{{ route('bill_query_courier_ajax') }}`,
                    data: function(d) {
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return '<div class="display-flex"><button class="btn btn-info btn-view-case" data-id="' +
                                row.id + '" data-encrypted-id="' + row.encrypted_id + '">View</button></div> ';
                        },
                        orderable: false
                    },
                ],
            ...
            $(document).on('click', '.btn-view-files', function() {
                ...
                // AJAX to fetch files
                $.ajax({
                    url: '/billing/query/files/' + caseId,
                    method: 'GET',
                    success: function(response) {
                        ...
                        allKeys.forEach(function(key) {
                            ...
                            html +=
                                '<span class="font-weight-bold mr-2" style="min-width:180px;">' +
                                label + ':</span>';
                            if (files[key]) {
                                var filename = files[key].split('/').pop();
                                html += getFileIcon(filename) +
                                    '<a href="/storage/' + files[key] +
                                    '" target="_blank" class="ml-2">View</a>' +
                                    '<a href="/storage/' + files[key] +
                                    '" download class="btn btn-primary btn-sm ml-2"><i class="fa fa-download"></i> Download</a>';
                            } else {
            ...
            $(document).on('change', '.upload-pdf-form input[type="file"]', function() {
                ...
                $.ajax({
                    url: '/billing/query/' + queryId + '/update-pdf',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
