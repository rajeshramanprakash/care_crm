                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex  align-items-center">
                                    <h3 class="card-title m-0">Staff List</h3>
                                    <input type="text" class="form-control" style="font-size: 14px; width: 60%; margin-left:20px; border-radius:4px 4px 4px 4px;" placeholder="Search staff..." id="searchInput">
                            ...
                            <div class="card-body p-0" style="height: calc(100vh - 100px); overflow-y: auto;">
                                <div class="list-group list-group-flush">
                                    ...
                                    <div class="list-group-item p-0">
                                        <div class="chat-user-item" data-user-id="{{ $staff->id }}">
                                            <div class="d-flex align-items-center p-3" style="cursor: pointer;"
                                                 data-bs-toggle="collapse"
                                                 data-bs-target="#chatList{{ $staff->id }}">
                                                <div class="position-relative">
                                                    <img src="{{ $staff->profile_image ? asset('storage/'.$staff->profile_image) : asset('images/default-user.png') }}"
                                                ...
                                                <div class="ms-3 flex-grow-1">
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <h6 class="mb-0" style="font-size: 14px">{{ $staff->f_name }} {{ $staff->l_name }}</h6>
                                                        <small class="text-muted">{{ $staff->role ? $staff->role->name : 'No Role' }}</small>
                                                    </div>
                                                    <small class="text-muted">{{ $staff->email }}</small>
                                                </div>
                                            </div>
                                            <div class="collapse" id="chatList{{ $staff->id }}">
                                                <div class="list-group list-group-flush">
                                                    @foreach($staff->chatParticipants as $participant)
                                                    <a href="#" class="list-group-item list-group-item-action chat-participant-link"
                                                       data-sales-id="{{ $staff->id }}"
                                                       ...
                                                       data-participant-name="{{ $participant->f_name }} {{ $participant->l_name }}">
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <div class="ms-3 flex-grow-1">
                                                                <div class="d-flex justify-content-between align-items-center">
                                                                    <h6 class="mb-0" style=" font-size:13px;">{{ $participant->f_name }} {{ $participant->l_name }}</h6>
                                                                ...
                                                                <div class="d-flex justify-content-between align-items-center">
                                                                    <div class="last-message" style=" font-size:12px;">
                                                                        @if($participant->lastMessage)
...
@section('footer-script')
    ...
    <script>
        ...
        function loadMessages(salesId, participantId) {
            $.get(`{{ route('manager.staff.chats.messages', ['salesId' => ':salesId', 'participantId' => ':participantId']) }}`.replace(':salesId', salesId).replace(':participantId', participantId))
                .done(function(messages) {
                    const messagesContainer = $('#chat-messages');
                    messagesContainer.empty();

                    messages.forEach(message => {
                        const isFromSales = message.sender_id == salesId;
                        const messageHtml = `
                            <div class="chat-message ${isFromSales ? 'outgoing' : 'incoming'}">
        ...
        function updateUnreadCounts() {
            $.get('{{ route('manager.staff.chats.unread-counts') }}')
        ...
        $(document).ready(function() {
            ...
            $('.sort-btn').click(function() {
                ...
                const $staffList = $('.list-group');
                const $staffMembers = $staffList.children('.list-group-item').get();
            ...
            $('.chat-participant-link').click(function(e) {
                ...
                selectedSalesId = $(this).data('sales-id');
                ...
                loadMessages(selectedSalesId, selectedParticipantId);

                // Mark messages as read
                $.ajax({
                    url: `{{ route('manager.staff.chats.mark-read', ['userId' => ':userId']) }}`.replace(':userId', selectedParticipantId),
                    method: 'POST',
                    headers: {
