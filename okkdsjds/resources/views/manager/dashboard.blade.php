...
@section('footer-script')
    ...
    <script>
        ...
        function editLeadFromRecentModal(leadId) {
            // Close the recent leads modal
            const recentModal = bootstrap.Modal.getInstance(document.getElementById('recentLeadsModalManager'));
            if (recentModal) {
                recentModal.hide();
            }
            
            // Redirect to the leads page (which will open in edit mode if needed)
            // Or you can trigger the edit modal directly
        ...
        function loadRecentCallsData() {
            ...
            fetch('{{ route("manager.dashboard.recent-calls") }}')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
        ...
        function loadPendingCallbacksData() {
            ...
            fetch(`{{ route('manager.dashboard.pending-callbacks') }}`)
                .then(response => response.json())
                .then(data => {
                    if (data.error || !data.success) {
                        tableBody.innerHTML = '<tr><td colspan="9" class="text-center text-muted"><i class="fas fa-exclamation-triangle fa-2x mb-3"></i><br>Error loading data</td></tr>';
                        return;
                    }
                ...
                .catch(error => {
                    ...
                    tableBody.innerHTML = '<tr><td colspan="9" class="text-center text-muted"><i class="fas fa-exclamation-triangle fa-2x mb-3"></i><br>Error loading pending callbacks</td></tr>';
                });
        }
        ...
        function updatePendingCallbacksDisplay(data) {
            ...
            data.leads.forEach(lead => {
                ...
                tableHTML += `<tr>
                    ...
                    <td>${lead.view_url ? `<a href="${lead.view_url}" class="btn btn-sm btn-outline-primary" target="_blank"><i class="fas fa-eye"></i></a>` : '<span class="text-muted">N/A</span>'}</td>
                </tr>`;
            });
            tableBody.innerHTML = tableHTML;
        }
        ...
        function loadPendingCallbacksCount() {
            fetch(`{{ route('manager.dashboard.pending-callbacks') }}`)
                ...
                .then(data => {
                    if (data.success) {
                        const cardCount = document.getElementById('pendingCallbacksCount');
                        if (cardCount) cardCount.textContent = data.data.total_pending || 0;
                    }
                })
                .catch(error => console.error('Error loading pending callbacks count:', error));
        }
