        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb-2 align-items-center">
                <div class="btn-group mb-3">
                    ...
                    @endforeach
                    <a href="{{ route('manager.users.manage') }}" class="btn add-user-btn">
                        <i class="fas fa-user-plus"></i> Add New User
                    </a>
                </div>                <div class="d-flex gap-2">
                    <a href="{{ route('manager.break_logs.index') }}" class="btn btn-info">
    ...
    <script>
        ...
        $(function() {
            var table = $('#serverTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('manager.users.getUsers') }}",
                    data: function(d) {
                        ...
                        render: function(data, type, row) {
                            return `
                                <div class="action-btns">
                                    <a href="/manager/break-logs/user/${row.id}" class="btn btn-info" title="Break Logs"><i class="fas fa-clock"></i></a>
                                    <a href="/manager/duty-logs/user/${row.id}" class="btn btn-success" title="Duty Logs"><i class="fas fa-user-check"></i></a>
                                </div>
                            `;
                        }
                    }
            ...
            $(document).on('click', '.delete-btn', function() {
                if (confirm('Are you sure you want to delete this user?')) {
                    ...
                    $.ajax({
                        url: "{{ route('manager.users.destroy', '') }}/" + userId,
                        type: 'get',
                        headers: {
