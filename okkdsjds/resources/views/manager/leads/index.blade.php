            ...
            $(document).on('click', '.view-btn', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Filter recordings by team members (users with same parent_id as auth user)
                        const authUserId = {{ Auth::user()->id }};
                        const teamMobileNumbers = {!! json_encode(\App\Models\User::where('parent_id', Auth::user()->id)->pluck('mobile')->toArray()) !!};

                        const filteredRecordings = recordingsArr.filter(function(rec) {
                            let meta = {};
                            try {
                                meta = rec.metadata ? JSON.parse(rec.metadata) : {};
                            } catch(e) {}
                            return meta.emp_phone && teamMobileNumbers.includes(meta.emp_phone);
                        });

                        if (Array.isArray(filteredRecordings) && filteredRecordings.length > 0) {
                            $('#view_recordings_group').show();
                            let html = '';
                            filteredRecordings.forEach(function(rec, idx) {
                                let meta = {};
