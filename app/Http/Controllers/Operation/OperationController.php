...
class OperationController extends Controller
{
    ...
    public function dashboard(Request $request){
        ...
        try {
            ...
            if ($request->expectsJson()) {
                $todayTasksCount = Task::where('assigned_to', $userId)
                    ->whereDate('due_date', $today)
                    ->count();

                $calendarFollowupCount = OperationLead::where('executive', $user->id)
                    ->where('status', 'follow up')
                    ->count();

                $pendingCallbacksCount = CallDetails::where('executive_id', $user->id)
                    ->whereIn('call_for', ['operation_lead', 'job_request'])
                    ->whereIn('call_status', ['no answer', 'noanswer', 'busy', 'failed', 'switchedoff', 'switched off'])
                    ->get()
                    ->unique(function ($item) {
                        return $this->normalizePhoneNumber($item->caller_id_number) . '_' . ($item->call_for ?? '');
                    })
                    ->count();

                $recentCallsCount = CallDetails::where('executive_id', $user->id)
                    ->whereIn('call_for', ['operation_lead', 'job_request'])
                    ->where('call_received_datetime', '>=', Carbon::now()->subWeek())
                    ->count();

                $totalVendorRemainingAmount = OperationDeploymentDetails::whereHas('operationLead', function ($query) use ($user) {
                        $query->where('executive', $user->id);
                    })
                    ->where('verify_payment', false)
                    ->whereNotNull('vendor_payment')
                ...
                return response()->json([
                    'success' => true,
                    'todayOperationLeadsCount' => $todayOperationLeads,
                    'totalOperationLeadsCount' => $totalOperationLeads,
                    ...
                    'outstandingPaymentsAmount' => $outstandingPayments,
                    'pendingDeploymentsCount' => $deploymentPending,
                    'myDeploymentPendingCount' => $myDeploymentPending,
                    'teamDeploymentPendingCount' => $teamDeploymentPending,
                    'profilePendingCount' => $profilePending,
                    'myProfilePendingCount' => $myProfilePending,
                    'teamProfilePendingCount' => $teamProfilePending,
                    'ongoingLeadsCount' => $ongoingLeads,
                    'unverifiedPaymentsCount' => $unverifiedPayments,
                    'jobRequestsCount' => $nonActiveJobRequests,
                    'unverifiedVendorFreelancerPaymentsCount' => $unverifiedVendorFreelancerPayments,
                    ...
                    'recentLeadsCount' => $recentOperationLeads->count(),
                ]);
            }
            ...
            \Log::error('Operation dashboard load failed', [
                'error' => $e->getMessage(),
            ...
            abort(500, 'Failed to load dashboard data');
        }
    }
    ...
    {
        try {
            $executives = OperationLead::join('users', 'operation_leads.executive', '=', 'users.id')
        ...
        try {
            ...
            $deployments = $unverifiedDeployments->map(function($deployment) {
                ...
                return [
                    ...
                    'verify_payment' => $deployment->verify_payment ?? false,
            ...
            foreach ($callLogs as $callLog) {
                ...
                if ($callLog->call_for === 'operation_lead') {
                    ...
                    if ($lead) {
                        ...
                        $lastUpdated = $lead->updated_at ? $lead->updated_at->toIso8601String() : null;
                    }
                    ...
                    if ($jobRequest) {
                        ...
                        $location = $jobRequest->location ?? $jobRequest->city;
                        $query = $jobRequest->city ?? 'Job Request';
                        $status = $jobRequest->status;
                        $lastUpdated = $jobRequest->updated_at ? $jobRequest->updated_at->toIso8601String() : null;
                    }
                }

                // Normalize lead_source from call_source to IVR / WhatsApp / Web
                $callSource = $callLog->call_source ?? '';
                $rawSource = is_string($callSource) ? strtolower(trim($callSource)) : '';
                if (strpos($rawSource, 'whatsapp') !== false || $rawSource === 'whatsapp') {
                    $leadSourceDisplay = 'WhatsApp';
                } elseif (strpos($rawSource, 'web') !== false || $rawSource === 'web') {
                    $leadSourceDisplay = 'Web';
                } elseif (strpos($rawSource, 'ivr') !== false || strpos($rawSource, 'ivrs') !== false
                    || strpos($rawSource, 'dial') !== false || strpos($rawSource, 'tata') !== false
                    || $rawSource === 'incoming call' || $rawSource === '') {
                    $leadSourceDisplay = 'IVR';
                } else {
                    $leadSourceDisplay = 'IVR';
                }
                ...
                // Last call date/time (when this missed call occurred)
                $callResponseDatetime = $callLog->call_received_datetime ? $callLog->call_received_datetime->toIso8601String() : null;
                ...
                $pendingCallbacks[] = [
                    ...
                    'call_response_datetime' => $callResponseDatetime,
