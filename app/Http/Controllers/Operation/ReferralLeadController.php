...
class ReferralLeadController extends Controller
{
    public function index()
    {
        ...
        return view('operation.referral_leads.index', compact('items', 'serviceOptions', 'referralLeadCommissionPercent'));
    }

    public function store(Request $request, SalesReferralLeadService $leadService)
    {
        try {
            $leadService->createLead(Auth::user(), $request->all(), SalesReferralLeadService::SOURCE_OPERATION);
        } catch (ValidationException $e) {
            return redirect()
                ->route('operation.referral_leads.index')
                ->withErrors($e->errors())
                ->withInput();
        }
