...
use App\Http\Controllers\Api\Concerns\ResolvesInsurerPortalUser;
...
class InsurerPortalApiController extends Controller
{
    use ManagesPartnerCorporateAccounts;
    use ResolvesInsurerPortalUser;

    private ?InsurerUser $resolvedInsurer = null;
    ...
    {
        return $this->resolvedInsurer?->id;
    }

    private function bootInsurer(Request $request): InsurerUser
    {
        return $this->resolvedInsurer = $this->insurerFromRequest($request);
    }
    ...
    private function insurerProfile(InsurerUser $insurer): array
    {
        $companyDocs = is_array($insurer->company_documents) ? array_values(array_filter($insurer->company_documents)) : [];
        ...
        return [
            ...
            'is_active' => (bool) $insurer->is_active,
            'portal_label' => 'Insurer Portal',
            'mou_file_url' => $this->storageUrl($insurer->mou_file),
    ...
    {
        $insurer = $this->bootInsurer($request);

        return response()->json([
            'profile' => $this->insurerProfile($insurer),
        ]);
    }
    ...
    public function updatePassword(Request $request)
    {
        ...
        if (! $insurer->checkPassword($data['current_password'])) {
            return response()->json([
                'success' => false,
        ...
        $insurer->password = $data['password'];
        $insurer->save();
