...
class CourierController extends Controller
{
    public function index($dashboard_filters = null)
    {
        ...
        return view('sales.courier.index', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
    }
    ...
    public function update_post_one(Request $request)
    {
        ...
        if ($request->post_status == 'Query') {
            ...
            $this->createTicketForQuery($query, $request->case_id, 'post_1');
        }
    ...
    public function update_post_two(Request $request)
    {
        ...
        if ($request->post_two_status == 'Query') {
            $query = new Query();
            $query->case_id = $request->case_id;
            $query->created_by = auth()->user()->id;
            ...
            // Create ticket for the query
            $this->createTicketForQuery($query, $request->case_id, 'post_2');
        }
    ...
    {
        try {
            \Log::info('Starting ticket creation for query: ' . $query->id . ' in Sales controller');
            ...
            if (!$vendor) {
                \Log::error('Vendor not found for case: ' . $caseId);
                return;
            }

            // Determine TPA based on case type
            $tpa = null;
            if ($caseType === 'post_1') {
            ...
            if (!$tpa) {
                \Log::info('TPA not assigned for case: ' . $caseId . '. Ticket creation skipped.');
                return;
            }
