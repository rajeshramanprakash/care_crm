...
use App\Events\DirectChatMessageCreated;
use App\Events\DirectChatMessagesRead;
...
class ChatController extends Controller
{
    public function index()
    {
        ...
        $sorted = $enrichedUsers->sort(function($a, $b) {
        ...
        return view('operation_manager.chat.index', ['users' => $sorted]);
    }
    ...
    public function getMessages($userId)
    {
        ...
        })->with(['repliedTo.sender'])->orderBy('created_at', 'asc')->get();
    ...
    public function sendMessage(Request $request)
    {
        ...
        \App\Support\SafeBroadcast::toOthers(new DirectChatMessageCreated($message));
        // Send Expo notification to receiver
        ExpoNotificationService::send([
            $request->receiver_id
        ], 'New Direct Message', $request->message);
        return response()->json($message->loadMissing(['sender', 'repliedTo.sender']));
    }
    ...
    public function markAsRead($userId)
    {
        Message::where('sender_id', $userId)
               ...
               ->where('is_read', false)
               ->update(['is_read' => true]);
        \App\Support\SafeBroadcast::toOthers(new DirectChatMessagesRead(Auth::id(), (int) $userId));
    ...
    public function translate(Request $request)
    {
        ...
        $translated = $gemini->translate($request->text, $request->target_language);
        if ($translated === null) {
            return response()->json(['success' => false, 'message' => 'Translation failed. Check GEMINI_API_KEY in .env'], 500);
        }
