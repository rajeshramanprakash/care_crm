...
class ChatController extends Controller
{
    public function index()
    {
        ...
        // Get all customers assigned to this freelancer via OperationDeploymentDetails
        ...
        $enrichedCustomers = $customers->map(function($customer) use ($freelancerId) {
            // Unread count (messages from customer to freelancer)
            ...
            $lastMsg = CustomerChatMessage::where(function($q) use ($customer, $freelancerId) {
                $q->where('sender_type', 'customer')
                  ->where('sender_id', $customer->id)
                  ->where('receiver_type', 'freelancer')
                  ->where('receiver_id', $freelancerId);
            })->orWhere(function($q) use ($customer, $freelancerId) {
        ...
        return view('freelancer.customer-chats', ['customers' => $sortedCustomers]);
    }
    ...
    public function getMessages($customerId, Request $request)
    {
        ...
        // Verify customer is assigned to this freelancer
        ...
        $query = CustomerChatMessage::where(function($q) use ($customerId, $freelancerId) {
            $q->where('sender_type', 'freelancer')
              ->where('sender_id', $freelancerId)
              ->where('receiver_type', 'customer')
        ...
        $calls = CustomerChatCall::where(function($q) use ($customerId, $freelancerId) {
    ...
    public function getCallOffer(Request $request)
    {
        ...
        // Check for incoming call offer (from customer)
        $cacheKey = "call_offer_customer_to_freelancer_{$freelancerId}_{$customerId}";
        ...
        if ($offer) {
            ...
            $callIdCacheKey = "call_id_customer_{$customerId}_freelancer_{$freelancerId}";
    ...
    public function sendCallOffer(Request $request)
    {
        ...
        // Log call initiation
        $callLog = CustomerChatCall::create([
            'caller_type' => 'freelancer',
            'caller_id' => $freelancerId,
            'receiver_type' => 'customer',
        ...
        // Store call ID in cache for later update
        $callIdCacheKey = "call_id_freelancer_{$freelancerId}_{$customerId}";
        \Illuminate\Support\Facades\Cache::put($callIdCacheKey, $callLog->id, 300);

        // Store offer in cache for customer to pick up (format: call_offer_freelancer_to_customer_{freelancerId}_{customerId})
        $cacheKey = "call_offer_freelancer_to_customer_{$freelancerId}_{$customerId}";
        \Illuminate\Support\Facades\Cache::put($cacheKey, $offer, 60);

        // Check for answer from customer
        $answerCacheKey = "call_answer_customer_to_freelancer_{$freelancerId}_{$customerId}";
    ...
    public function getCallAnswer(Request $request)
    {
        ...
        $cacheKey = "call_answer_customer_to_freelancer_{$freelancerId}_{$customerId}";
    ...
    public function callAnswer(Request $request)
    {
        ...
        // Update call status to connected if call was initiated by freelancer
        $callIdCacheKey = "call_id_freelancer_{$freelancerId}_{$customerId}";
        $callId = \Illuminate\Support\Facades\Cache::get($callIdCacheKey);
        if ($callId) {
            CustomerChatCall::where('id', $callId)
                ->where('caller_type', 'freelancer')
                ->where('caller_id', $freelancerId)
                ->where('receiver_type', 'customer')
                ->where('receiver_id', $customerId)
        ...
        $incomingCallIdCacheKey = "call_id_customer_{$customerId}_freelancer_{$freelancerId}";
        ...
        $cacheKey = "call_answer_freelancer_{$freelancerId}_{$customerId}";
    ...
    public function getCallIce(Request $request)
    {
        ...
        $cacheKey = "call_ice_customer_to_freelancer_{$freelancerId}_{$customerId}";
    ...
    public function sendCallIce(Request $request)
    {
        ...
        // Store ICE candidate for customer to retrieve (format: call_ice_freelancer_{freelancerId}_{customerId})
        $cacheKey = "call_ice_freelancer_{$freelancerId}_{$customerId}";
        $candidates = \Illuminate\Support\Facades\Cache::get($cacheKey, []);
    ...
    public function callEnd(Request $request)
    {
        ...
        $keys = [
            "call_offer_freelancer_{$freelancerId}_{$customerId}",
            "call_offer_customer_to_freelancer_{$freelancerId}_{$customerId}",
            "call_offer_freelancer_to_customer_{$freelancerId}_{$customerId}",
            "call_answer_freelancer_{$freelancerId}_{$customerId}",
            "call_answer_customer_to_freelancer_{$freelancerId}_{$customerId}",
            "call_ice_freelancer_{$freelancerId}_{$customerId}",
            "call_ice_customer_to_freelancer_{$freelancerId}_{$customerId}"
        ];
        foreach ($keys as $key) {
