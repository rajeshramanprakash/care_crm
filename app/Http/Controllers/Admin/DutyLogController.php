    ...
    public function index()
    {
        ...
        if (request()->expectsJson()) {
            ...
            foreach ($dutyLogs as $log) {
                $userName = $log->user ? "{$log->user->f_name} {$log->user->l_name}" : "User #{$log->user_id} (Deleted)";
                \Illuminate\Support\Facades\Log::info("Log ID: {$log->id}, User: {$userName}, Status: {$log->break_status}, Start: {$log->break_start_time}, End: {$log->break_end_time}");
            }
