    ...
    public function index()
    {
        ...
        foreach($salesStaff as $staff) {
            ...
            foreach($staff->chatParticipants as $participant) {
                $lastMessage = Message::where(function($query) use ($staff, $participant) {
                ...
                $participant->lastMessage = $lastMessage;

                $unreadCount = Message::where('sender_id', $participant->id)
                                    ->where('receiver_id', $staff->id)
                                    ->where('is_read', false)
                                    ->count();
        ...
        return view('admin.staff_chats.index', compact('salesStaff'));
    }
