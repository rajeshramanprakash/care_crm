...
class CaseController extends Controller
{
    public function index($dashboard_filters = null)
    {
        ...
        if ($dashboard_filters == 'main_claim_cases_approved' || $dashboard_filters == 'main_claim_cases_inprocess' || $dashboard_filters == 'main_claim_cases_paid'){
            return view('admin.case.index_two', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
            ...
            return view('admin.case.index_three', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
            ...
            return view('admin.case.index_post', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
            ...
            return view('admin.case.index_post_two', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
        }else{
            return view('admin.case.index', compact('page_heading', 'filter_params', 'vendors', 'tpa_roles'));
        }
    }
    ...
    public function ajax_list(Request $request)
    {
        ...
        return dataTables()->of($cases)
            ...
            ->addColumn('actions', function ($case) {
                return '<a href="' . route('admin.case.show', CaseIdEncryption::routeParam($case->id)) . '" class="btn btn-info">View</a>';
            })
            ->make(true);
    }
    ...
    public function show($encryptedId)
    {
        ...
        return view('admin.case.show', compact('case', 'tpa_roles', 'query_status', 'cases_with_same_memeber_id'));
    }
    ...
    public function post_hold(Request $request)
    {
        $request->validate([
            'case_id' => 'required|exists:cases,id',
            'hold_reason' => 'required|string|max:1000',
            'post_type' => 'required|in:post_1,post_2'
        ]);

        $case = Cases::findOrFail($request->input('case_id'));
        
        if ($request->input('post_type') == 'post_1') {
            $case->post_status = 'Hold';
            $case->post_hold_reason = $request->input('hold_reason');
        } else {
            $case->post_two_status = 'Hold';
            $case->post_two_hold_reason = $request->input('hold_reason');
        }
        
        $case->save();

        return response()->json(['message' => 'Case held successfully.']);
    }

    public function post_unhold(Request $request)
    {
        $request->validate([
            'case_id' => 'required|exists:cases,id',
            'post_type' => 'required|in:post_1,post_2'
        ...
        if ($request->input('post_type') == 'post_1') {
            ...
            $case->post_hold_reason = null;
        } else {
            $case->post_two_status = null;
            $case->post_two_hold_reason = null;
        }
        
        $case->save();

        return response()->json(['message' => 'Case unheld successfully.']);
    }
}
